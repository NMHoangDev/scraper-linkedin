...
export class Listener extends EventEmitter<ListenerEvents> {
    ...
    private autoReconnect: AutoReconnectOptions;
    private reconnectAttempts = 0;
    private reconnectTimer?: Timer;
    private isManualStop = false;

    // Message tracking for gap detection
    private lastUserMsgId: string | null = null;
    private lastGroupMsgId: string | null = null;

    // Promise resolvers for async old message requests
    private oldMessagesResolvers: Map<string, {
    ...
    constructor(
        ...
        private urls: string[],
        autoReconnectOptions?: Partial<AutoReconnectOptions>,
    ) {
        super();
    ...
    public start({ retryOnClose = false }: { retryOnClose?: boolean } = {}) {
        ...
        ws.onclose = (event) => {
            ...
            // Don't reconnect on manual stop or duplicate connection
            if (this.isManualStop || event.code === CloseReason.DuplicateConnection) {
                this.onClosedCallback(event.code, event.reason);
                this.emit("closed", event.code as CloseReason, event.reason);
                return;
            }

            // Try built-in retry first
            ...
            } else if (this.autoReconnect.enabled) {
                // Fall back to auto-reconnect with exponential backoff
                this.scheduleReconnect(retryOnClose);
            } else {
                this.onClosedCallback(event.code, event.reason);
                this.emit("closed", event.code as CloseReason, event.reason);
            }
        ...
        ws.onmessage = async (event) => {
            ...
            try {
                ...
                if (version == 1 && cmd == 501 && subCmd == 0) {
                    ...
                    for (const msg of msgs) {
                        if (typeof msg.content == "object" && hasOwn(msg.content, "deleteMsg")) {
                            ...
                            if (msg.msgId) this.lastUserMsgId = msg.msgId;
                ...
                if (version == 1 && cmd == 521 && subCmd == 0) {
                    ...
                    for (const msg of groupMsgs) {
                        if (typeof msg.content == "object" && hasOwn(msg.content, "deleteMsg")) {
                            ...
                            if (messageObject.isSelf && !this.selfListen) continue;
                            // Track last message ID for gap detection
                            if (msg.msgId) this.lastGroupMsgId = msg.msgId;
                            this.onMessageCallback(messageObject);
                            this.emit("message", messageObject);
                        }
                    }
                ...
                if (cmd == 510 && subCmd == 1) {
                    ...
                    // Resolve async request if pending
                    const userResolver = this.oldMessagesResolvers.get(`old_messages_${ThreadType.User}`);
                    if (userResolver) {
                        clearTimeout(userResolver.timeout);
                        this.oldMessagesResolvers.delete(`old_messages_${ThreadType.User}`);
                        userResolver.resolve(responseMsgs);
                    }
                }
                ...
                if (cmd == 511 && subCmd == 1) {
                    ...
                    // Resolve async request if pending
                    const groupResolver = this.oldMessagesResolvers.get(`old_messages_${ThreadType.Group}`);
                    if (groupResolver) {
                        clearTimeout(groupResolver.timeout);
                        this.oldMessagesResolvers.delete(`old_messages_${ThreadType.Group}`);
                        groupResolver.resolve(responseMsgs);
                    }
                }
    ...
    public stop() {
        ...
        for (const [key, resolver] of this.oldMessagesResolvers) {
            clearTimeout(resolver.timeout);
            resolver.reject(new Error("Listener stopped"));
            this.oldMessagesResolvers.delete(key);
        }
    }
     ...
     */
    private scheduleReconnect(retryOnClose: boolean) {
        if (this.autoReconnect.maxRetries > 0 && this.reconnectAttempts >= this.autoReconnect.maxRetries) {
            logger(this.ctx).error(`Auto-reconnect: max retries (${this.autoReconnect.maxRetries}) reached`);
            this.onClosedCallback(CloseReason.AbnormalClosure, "Max reconnect retries reached");
            this.emit("closed", CloseReason.AbnormalClosure, "Max reconnect retries reached");
            return;
        }

        this.reconnectAttempts++;
        const delay = Math.min(
            this.autoReconnect.initialDelay * Math.pow(this.autoReconnect.backoffMultiplier, this.reconnectAttempts - 1),
            this.autoReconnect.maxDelay,
        );

        logger(this.ctx).info(`Auto-reconnect: attempt ${this.reconnectAttempts} in ${delay}ms`);
        this.emit("reconnecting", this.reconnectAttempts, delay);

        this.reconnectTimer = setTimeout(() => {
            // Reset internal retry counts for fresh start
            for (const key in this.retryCount) {
                this.retryCount[key].count = 0;
            }
            this.rotateCount = 0;
            ...
            try {
                ...
                logger(this.ctx).error("Auto-reconnect failed:", error);
                this.scheduleReconnect(retryOnClose);
            }
        }, delay);
    }
    ...
    /**
     * Request old messages and return a Promise
     *
     * @param threadType ThreadType.User or ThreadType.Group
     * @param lastMsgId Last message ID for pagination (optional)
     * @param timeoutMs Timeout in ms (default: 10000)
     * @returns Promise resolving to array of messages
     */
    public requestOldMessagesAsync(
        threadType: ThreadType,
        lastMsgId: string | null = null,
        timeoutMs: number = 10000,
    ): Promise<Message[]> {
        return new Promise((resolve, reject) => {
            const key = `old_messages_${threadType}`;

            // Cancel any existing request for same type
            const existing = this.oldMessagesResolvers.get(key);
            if (existing) {
            ...
            const timeout = setTimeout(() => {
                this.oldMessagesResolvers.delete(key);
                reject(new ZaloApiError(`requestOldMessagesAsync timed out after ${timeoutMs}ms`));
            }, timeoutMs);

            this.oldMessagesResolvers.set(key, { resolve, reject, timeout });
            this.requestOldMessages(threadType, lastMsgId);
        });
    }

    /**
     * Request old reactions and return a Promise
     *
     * @param threadType ThreadType.User or ThreadType.Group
     * @param lastMsgId Last message ID for pagination (optional)
     * @param timeoutMs Timeout in ms (default: 10000)
     * @returns Promise resolving to array of reactions
     */
    public requestOldReactionsAsync(
        threadType: ThreadType,
        lastMsgId: string | null = null,
        timeoutMs: number = 10000,
    ): Promise<Reaction[]> {
        return new Promise((resolve, reject) => {
            const key = `old_reactions_${threadType}`;
            const timeout = setTimeout(() => {
                this.removeListener("old_reactions", handler);
                reject(new ZaloApiError(`requestOldReactionsAsync timed out after ${timeoutMs}ms`));
            }, timeoutMs);

            const handler = (reactions: Reaction[], isGroup: boolean) => {
                const expectedGroup = threadType === ThreadType.Group;
            ...
            this.on("old_reactions", handler);
            this.requestOldReactions(threadType, lastMsgId);
        });
    }
    ...
    /**
     * Get last known message IDs (useful for gap detection after reconnect)
     */
    public getLastMessageIds() {
        return {
            user: this.lastUserMsgId,
            group: this.lastGroupMsgId,
        };
    }
     ...
     */
    public setAutoReconnect(options: Partial<AutoReconnectOptions>) {
        Object.assign(this.autoReconnect, options);
    }
}
