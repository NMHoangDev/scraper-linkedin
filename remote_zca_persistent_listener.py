...
_ENABLE_BACKGROUND_BACKFILL = os.getenv("ZCA_ENABLE_BACKGROUND_BACKFILL", "").strip().lower() in {"1", "true", "yes", "on"}
_SHARED_CONVERSATION_CACHE_TTL_S = 30.0
...
class ListenerState:
    ...
    shared_conversation_ids_loaded_at: float = 0.0
...
class ZcaPersistentListenerManager:
            ...
            try:
                ...
                for idx, group_id in enumerate(group_ids):
                    ...
                    try:
                        ...
                        if not is_group:
                            logger.debug(
                                f"Skipping startup sync history for personal chat user={state.user_id} "
                                f"group={group_id}; realtime listener will capture new messages"
                            )
                            continue
                ...
                if state.desired and state.connected and _ENABLE_BACKGROUND_BACKFILL:
                ...
                elif state.desired and state.connected:
                    logger.info(
                        f"ZCA background backfill disabled for user={state.user_id}; "
                        "realtime listener remains prioritized"
                    )
    ...
    async def _run_background_backfill(self, state: ListenerState, exclude_group_ids: List[str]) -> None:
        ...
        try:
            ...
            for idx, group_id in enumerate(group_ids):
                ...
                if not is_group:
                    logger.debug(
                        f"Skipping background backfill history for personal chat user={state.user_id} "
                        f"group={group_id}; unsupported by current ZCA bridge"
                    )
                    continue
    ...
    async def _handle_event(self, state: ListenerState, raw_line: str) -> None:
        ...
        if event_name == "message":
            ...
            emitted_ts = event.get("ts")
            await self._record_messages(state, [event.get("message") or {}])
            elapsed_ms = int((asyncio.get_running_loop().time() - started_at) * 1000)
            if elapsed_ms > 2000:
                logger.warning(
                    f"Slow ZCA realtime message handling user={state.user_id} "
                    f"elapsed_ms={elapsed_ms} emitted_ts={emitted_ts}"
                )
            return
        if event_name == "old_messages":
            await self._record_messages(state, event.get("messages") or [], increment_unread=False)

    async def _get_shared_conversation_ids(self, state: ListenerState) -> Set[str]:
        now = asyncio.get_running_loop().time()
        if (
            state.shared_conversation_ids is not None
            and now - state.shared_conversation_ids_loaded_at < _SHARED_CONVERSATION_CACHE_TTL_S
        ):
            return state.shared_conversation_ids
        ...
        state.shared_conversation_ids = shared_ids
        state.shared_conversation_ids_loaded_at = now
        return shared_ids
    ...
    async def _record_messages(self, state: ListenerState, rows: List[Dict[str, Any]], *, increment_unread: bool = True) -> None:
        ...
        for group_id, messages in grouped.items():
            ...
            # Startup sync/backfill persists history but must not be pushed as
            # realtime SSE, otherwise old messages can flood the UI and delay
            # fresh incoming messages for active users.
            ...
            try:
                ...
                # Cache permissions briefly so realtime path does not hit
                # Supabase for every single incoming message.
                shared_ids = await self._get_shared_conversation_ids(state)
