    ...
    *,
    ...
    try:
        while True:
            ...
            # Cancel các task chưa xong (timeout) và await để không tích lũy
            # pending tasks khi client disconnect/reconnect nhiều lần.
            cancelled_tasks = []
            for task in pending:
                if not task.done():
                    task.cancel()
        ...
        # Nếu client disconnect đúng lúc đang await queue, hủy/await các task
        # q.get() còn pending để tránh "Task was destroyed but it is pending".
        cancelled_tasks = []
        for task in pending:
            if not task.done():
                task.cancel()
