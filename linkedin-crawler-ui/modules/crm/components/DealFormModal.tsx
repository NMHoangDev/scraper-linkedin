...
export function DealFormModal({
  ...
  const selectedSourceLabel = useMemo(
    () => SOURCE_OPTIONS.find(option => option.value === form.sourcePlatform)?.label || form.sourcePlatform,
    [form.sourcePlatform]
  );
  ...
  function buildPayload(): CreateDealInput | UpdateDealInput {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (!form.email.trim() && !form.phone.trim()) {
      window.alert('Cần nhập email hoặc số điện thoại để tạo contact.');
      return;
    }
      ...
      <div className="crm-modal" onClick={event => event.stopPropagation()}>
        <header className="crm-modal-header">
          <div>
            ...
            <p className="crm-modal-subtitle">Nguồn: {selectedSourceLabel}</p>
          </div>
