...
import {
  extractLinkedInMetadata,
  sanitizeLinkedInUrl,
} from "@/lib/linkedin-metadata";
...
const getPlatformIcon = (url?: string) => {
  if (!url) {
    return (
      <div
        className="w-8 h-8 rounded-full bg-gray-100 text-gray-500 grid place-items-center shrink-0 shadow-xs"
        title="Link"
      >
  ...
  if (lowerUrl.includes("linkedin.com") || lowerUrl.includes("lnkd.in")) {
    return (
      <div
        className="w-8 h-8 rounded-full bg-[#0a66c2] text-white grid place-items-center shrink-0 shadow-xs"
        title="LinkedIn"
      >
  ...
  if (lowerUrl.includes("youtube.com") || lowerUrl.includes("youtu.be")) {
    return (
      <div
        className="w-8 h-8 rounded-full bg-[#ff0000] text-white grid place-items-center shrink-0 shadow-xs"
        title="YouTube"
      >
  ...
  if (lowerUrl.includes("tiktok.com")) {
    return (
      <div
        className="w-8 h-8 rounded-full bg-black text-white grid place-items-center shrink-0 shadow-xs"
        title="TikTok"
      >
  ...
  return (
    <div
      className="w-8 h-8 rounded-full bg-[#1877f2] text-white grid place-items-center shrink-0 shadow-xs"
      title="Facebook"
    >
...
const isLinkedInUrl = (url: string): boolean =>
  /linkedin\.com|lnkd\.in/i.test(url);
...
export default function InternalEngagementPage() {
  ...
  const [marks, setMarks] = useState<
    Record<string, InternalEngagementMarkStatus>
  >({});
  const [dbTeams, setDbTeams] = useState<
    Array<{ id: string; name_team: string; member_count: number }>
  >([]);
  ...
  const [liAutoFetchStatus, setLiAutoFetchStatus] = useState<
  ...
  const [modalPost, setModalPost] = useState<InternalEngagementPost | null>(
    null,
  );
  ...
  const applyMetricsToState = (postId: string, updatedData: any) => {
    const newLikes =
      updatedData?.public_likes ?? updatedData?.fb_total_likes ?? 0;
    const newComments =
      updatedData?.public_comments ?? updatedData?.fb_total_comments ?? 0;
    const newShares =
      updatedData?.public_shares ?? updatedData?.fb_total_shares ?? 0;
    const newSyncedAt =
      updatedData?.synced_at ??
      updatedData?.last_synced_at ??
  ...
  const handleSyncPostMetrics = async (post: InternalEngagementPost) => {
    ...
    try {
      ...
      if (res && res.success) {
        const { newLikes, newComments, newShares } = applyMetricsToState(
          post.id,
          res.data,
        );
        showToast(
        ...
        showToast(
          res?.message || "Không thể cào dữ liệu từ bài viết Facebook này.",
          "error",
        );
      }
    } catch (err: any) {
      const errorMsg =
        ...
        err?.message ||
        "Có lỗi xảy ra khi đồng bộ bài viết Facebook.";
  ...
  const handleSyncLinkedInMetrics = async (post: InternalEngagementPost) => {
    ...
    if (!link)
      return showToast("Bài viết không có link LinkedIn hợp lệ.", "error");
    ...
    try {
      ...
      try {
        const serverRes =
          await internalEngagementService.syncLinkedInPlaywright(post.id);
      ...
      if (!fetched.success) {
        return showToast(
          fetched.error || "Không lấy được số liệu bài viết LinkedIn này.",
          "error",
        );
      }
      ...
      if (res && res.success) {
        ...
        showToast(
          res?.message || "Không thể lưu số liệu bài viết LinkedIn này.",
          "error",
        );
      }
    } catch (err: any) {
      const errorMsg =
        err?.response?.data?.message ||
        err?.message ||
        "Có lỗi xảy ra khi đồng bộ bài viết LinkedIn.";
      showToast(errorMsg, "error");
    } finally {
      setSyncingPostId(null);
    }
  ...
  // Team visibility
  const canSeeTeamInteractions =
    user?.role === "admin" || user?.role === "leader";
  const [teamCounts, setTeamCounts] = useState<
    Record<string, InternalEngagementPostTeamCount[]>
  >({});
  const [interactionsPost, setInteractionsPost] =
    useState<InternalEngagementPost | null>(null);
  const [interactionsRole, setInteractionsRole] = useState<string>("member");
  const [interactionsTeams, setInteractionsTeams] = useState<
    InternalEngagementTeamRef[]
  >([]);
  const [interactionsItems, setInteractionsItems] = useState<
    InternalEngagementInteraction[]
  >([]);
  const [interactionsTeamFilter, setInteractionsTeamFilter] =
    useState<string>("all");
  const [interactionsStatusFilter, setInteractionsStatusFilter] =
    useState<string>("all");
  ...
  const [editingPost, setEditingPost] = useState<InternalEngagementPost | null>(
    null,
  );
  ...
  const openEditModal = (post: InternalEngagementPost) => {
    ...
    setEditingPost(post);
    setEditFanpageName(
      (post as any).fanpage_name || (post as any).fanpageName || "",
    );
    ...
    const rawCampId =
      (post as any).campaign_id || (post as any).campaignId || "";
    setEditCampaignId(rawCampId);

    // Bắt mọi thể loại tên Deadline
    let formattedDeadline = "";
    const rawDeadline =
      (post as any).deadline || (post as any).due_date || (post as any).dueDate;
    if (rawDeadline) {
      try {
    ...
    const rawAssigned =
      (post as any).assigned_team_ids ||
      (post as any).assignedTeamIds ||
      (post as any).team_ids ||
      (post as any).teamIds ||
      [];
    ...
    const targetComm =
      Number((post as any).target_comments) ||
      Number((post as any).targetComments) ||
      0;
    if (targetComm > 0) {
      ...
      const initialCalc = dbTeams.reduce((sum, t) => {
        const isSelected =
          assignedList.includes(t.id) || assignedList.includes(t.name_team);
  ...
  const handleSaveEdit = async () => {
    ...
    if (
      isNaN(targetComm) ||
      targetComm <= 0 ||
      targetComm > editTotalSelectedMembers
    ) {
      return showToast(
    ...
    try {
      ...
      let res = await internalEngagementService.updateCustomPost(
        editingPost.id,
        payload,
      );
      if (!res || !res.success) {
        res = await internalEngagementService.overrideMarkeePost(
          editingPost.id,
          payload,
        );
      }
      ...
      if (res && res.success) {
        showToast(
          "Đã cập nhật thông tin bài viết & Target KPI thành công!",
          "success",
        );
        setEditingPost(null);
        await loadPosts();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (deletingPost.fanpage_id === "custom") {
        res = await internalEngagementService.deleteCustomPost(
          deletingPost.id,
          user.email,
        );
      } else {
        res = await internalEngagementService.hideMarkeePost(
          deletingPost.id,
          user.email,
        );
      }
  ...
  const { libraryItems: commentTemplates } = useQuickCommentLibrary(
    modalPost?.platform || "facebook",
  );
  ...
  const handleDoTask = async (post: InternalEngagementPost) => {
    ...
    if (allQuickComments && allQuickComments.length > 0) {
      ...
      const candidateList =
        matchingTemplates.length > 0 ? matchingTemplates : allQuickComments;
      const randomItem =
      ...
      if (randomItem && randomItem.content) {
        try {
          ...
          showToast(
            "Đã copy mẫu câu. Hãy dán (Ctrl+V) vào bình luận nhé!",
            "success",
          );
        } catch (err) {
    ...
    const facebookPostId =
      (post as any).facebook_post_id ||
      (post as any).id_post ||
      (post as any).facebookPostId ||
      post.id ||
      "unknown";
  ...
  const commentTemplateGroups = useMemo(() => {
    const groups = new Map<
      string,
      { label: string; templates: typeof commentTemplates }
    >();
    ...
    try {
      ...
      if (user?.email) {
        ...
        if (allLinks.length > 0) {
          const marksRes = await internalEngagementService.getMyMarks(
            user.email,
            allLinks,
          );
  ...
  const loadTeams = async () => {
    try {
      ...
      const members =
        membersRes && membersRes.success && Array.isArray(membersRes.data)
          ? membersRes.data
          : [];
      ...
      const rawTeamsList =
        teamsRes && teamsRes.success && teamsRes.data
          ? teamsRes.data.teams ||
      ...
      if (rawTeamsList.length > 0) {
        const parsedTeams = rawTeamsList.map((t: any) => {
          ...
          const mapCount =
            countByTeamKeyMap[tId] || countByTeamKeyMap[tName] || 0;
  ...
  const totalSelectedMembers = useMemo(() => {
    ...
    if (dbTeams.length > 0) {
      return dbTeams.reduce((sum, t) => {
        const isSelected =
          taskAssignedTeams.includes(t.id) ||
          taskAssignedTeams.includes(t.name_team);
        return sum + (isSelected ? t.member_count || 0 : 0);
      }, 0);
    }
  ...
  const editTotalSelectedMembers = useMemo(() => {
    ...
    if (dbTeams.length > 0) {
      return dbTeams.reduce((sum, t) => {
        const isSelected =
          editAssignedTeams.includes(t.id) ||
          editAssignedTeams.includes(t.name_team);
        return sum + (isSelected ? t.member_count || 0 : 0);
      }, 0);
    }
  ...
  const [deletingCampaignTarget, setDeletingCampaignTarget] = useState<{
    id: string;
    name: string;
  } | null>(null);
    ...
    try {
      const res = await internalEngagementService.createCampaign({
        ...
        start_date: newCampaignStartDate
          ? new Date(newCampaignStartDate).toISOString()
          : undefined,
        end_date: newCampaignEndDate
          ? new Date(newCampaignEndDate).toISOString()
          : undefined,
  ...
  const taskLinkedInMetricsRef = useRef<{
    likes?: number;
    comments?: number;
    shares?: number;
  }>({});
  ...
  const autoFetchLiInfo = async (link: string, opts?: { silent?: boolean }) => {
    ...
    const isStale = () =>
      lastAutoFetchedLiLinkRef.current !== link &&
      lastAutoFetchedLiLinkRef.current !== cleanLink;
    ...
    try {
      ...
      if (res && res.success && res.data) {
        ...
        if (!opts?.silent) {
          showToast(
            "Đã tự động lấy nội dung bài viết LinkedIn thành công!",
            "success",
          );
        }
      } else {
  ...
  const handleAutoFetchLiInfo = () => {
    ...
    if (!trimmed)
      return showToast("Vui lòng dán link bài viết LinkedIn trước.", "error");
  ...
  const handleCreateTaskSubmit = async () => {
    const socialRegex =
      /^(https?:\/\/)?([\w-]+\.)*(facebook\.com|fb\.com|fb\.watch|youtube\.com|youtu\.be|tiktok\.com|linkedin\.com|lnkd\.in)\/.+$/i;
    ...
    if (!socialRegex.test(rawLink)) {
      return showToast(
        "Vui lòng nhập đường link hợp lệ (Facebook, YouTube, TikTok, LinkedIn).",
        "error",
      );
    }
    ...
    if (
      isNaN(targetComm) ||
      targetComm <= 0 ||
      targetComm > totalSelectedMembers
    ) {
      return showToast(
    ...
    const finalCleanLink = isLinkedInLink
      ? sanitizeLinkedInUrl(rawLink)
      : rawLink;

    if (isLinkedInLink && liAutoFetchStatus === "fetching") {
      return showToast(
        "Đang tự động lấy nội dung bài viết, vui lòng đợi vài giây...",
        "error",
      );
    }
    ...
    try {
      ...
      const payload = {
        ...
        fanpage_name: isLinkedInLink
          ? taskLinkedInAuthor.trim() || undefined
          : undefined,
        likes: isLinkedInLink
          ? taskLinkedInMetricsRef.current.likes
          : undefined,
        comments: isLinkedInLink
          ? taskLinkedInMetricsRef.current.comments
          : undefined,
        shares: isLinkedInLink
          ? taskLinkedInMetricsRef.current.shares
          : undefined,
  ...
  const handleRemindMembers = (post: InternalEngagementPost) => {
    showToast(
      "Đã gửi thông báo nhắc nhở tới các thành viên chưa hoàn thành bài viết!",
      "success",
    );
  };
  ...
  const handleSubmitCustomLink = async () => {
    const fbRegex =
      /^(https?:\/\/)?(www\.|m\.|mobile\.)?(facebook\.com|fb\.com|fb\.watch)\/.+$/i;
    ...
    if (!fbRegex.test(customLink.trim())) {
      return showToast(
        "Vui lòng nhập đường link Facebook hợp lệ (facebook.com).",
        "error",
      );
    }
    ...
    try {
      const res = await internalEngagementService.addCustomPost(
        customLink.trim(),
        user.email,
      );
      if (res && res.success) {
  ...
  const realStats = useMemo(() => {
    ...
    const totalSeeder =
      dbTeams.reduce((sum, t) => sum + (t.member_count || 0), 0) ||
      (membersCount > 0 ? membersCount : 20);
    ...
    const totalTargetComments = allCombinedPosts.reduce((sum, post) => {
      ...
      const teams = ((post as any).assigned_team_ids || []).map((t: string) =>
        t?.toLowerCase(),
      );
      if (teams.length === 0) {
        ...
        const assignedDbTeams = dbTeams.filter(
          (t) =>
            teams.includes(t.id.toLowerCase()) ||
            teams.includes(t.name_team.toLowerCase()),
        );
        const postTarget = assignedDbTeams.reduce(
          (s, t) => s + (t.member_count || 0),
          0,
        );
      ...
      const postInteracted = postTeamStats
        .filter(
          (t) =>
            ...
            assignedTeams.includes(t.team_id) ||
            assignedTeams.includes(t.team_name),
        )
        .reduce((s, t) => s + (t.count || 0), 0);
      return sum + postInteracted;
    }, 0);

    const activeUniqueMembers = leaderboard.filter(
      (item) => item.total_completed > 0,
    ).length;

    const perfectMembers = leaderboard.filter(
      (item) =>
        item.total_completed === item.total_assigned && item.total_assigned > 0,
    ).length;
  ...
  const filteredPosts = useMemo(() => {
    return allCombinedPosts.filter((post) => {
      ...
      if (search.trim()) {
        ...
        const fanpageMatch = (post.fanpage_name || "")
          .toLowerCase()
          .includes(q);
      ...
      if (selectedTeamFilter !== "all") {
        ...
        if (assignedTeams.length > 0) {
          const hasTeam = assignedTeams.some(
            (t) =>
              t === selectedTeamFilter ||
              t.toLowerCase() === targetTeamName.toLowerCase(),
          );
      ...
      // 4. Status Filter
      const rawTarget =
        (post as any).target_comments || (post as any).targetComments;
      const targetTotal = Number(rawTarget) > 0 ? Number(rawTarget) : 32;
      const postTeamStats = teamCounts[post.id] || [];
      const interactedCount = postTeamStats.reduce(
        (sum, t) => sum + (t.count || 0),
        0,
      );
      ...
      const isOverdue =
        ...
        new Date((post as any).deadline) < new Date() &&
        !isCompleted;
      ...
      allCombinedPosts
        ...
        .map((p: InternalEngagementPost) =>
          internalEngagementService
            ...
            .then(
              (res) =>
                [p.id, res.success && res.data ? res.data.teams : []] as const,
            ),
        ),
    ).then((results) => {
      if (cancelled) return;
  ...
  const openInteractionsModal = async (
    post: InternalEngagementPost,
    teamId?: string,
  ) => {
  ...
  const lastResultRef = useRef<{ success: boolean; error?: string } | null>(
    null,
  );
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (action === "COMMENT_EXTENSION_READY") {
        ...
        if (result && result.success === false) {
          showToast(
            `Comment thất bại: ${result.error || "Lỗi không xác định"}`,
          );
        } else {
        ...
        setRunProgress(event.data.payload?.status || null);
        if (event.data.payload?.result)
          lastResultRef.current = event.data.payload.result;
        ...
        if (lastResultRef.current && lastResultRef.current.success === false) {
          showToast(
            `Comment thất bại: ${lastResultRef.current.error || "Lỗi không xác định"}`,
          );
        } else {
          showToast("Đã gửi comment thành công. Hệ thống đã ghi nhận KPI.");
        }
  ...
  const sendComment = () => {
    ...
    if (!commentText.trim())
      return showToast("Vui lòng nhập nội dung comment.");
    if (!user?.email) {
      return showToast(
        "Chưa xác định được tài khoản đăng nhập — tải lại trang trước khi comment (nếu không KPI sẽ không được ghi nhận).",
      );
    }
    ...
    const isLinkedIn =
      ...
      Boolean(
        ...
        (modalPost.permalink_url.includes("linkedin.com") ||
          modalPost.permalink_url.includes("lnkd.in")),
      );
    const isReady = isLinkedIn
      ? isLiExtensionReady || isExtensionReady
      : isExtensionReady;
    if (!isReady)
      return showToast(
        "Chưa kết nối được Extension. Vui lòng cài đặt và tải lại trang.",
      );

    const rawPostLink = modalPost.permalink_url || (modalPost as any).link_post;
    const targetLink = isLinkedIn
      ? sanitizeLinkedInUrl(rawPostLink)
      : rawPostLink;
  ...
  const activeLeaderboard = useMemo(() => {
    ...
    const filtered = leaderboard.filter((item: any) => {
      const teamName = String(item.team_name || item.team || "")
        .toLowerCase()
        .trim();
      if (
        !teamName ||
        teamName === "null" ||
        teamName.includes("chưa phân team")
      ) {
        return false;
      }
      return true;
  ...
  const leaderboardTotalPages = Math.ceil(
    activeLeaderboard.length / LEADERBOARD_PAGE_SIZE,
  );
  ...
  return (
    <div className="w-full bg-[#f7f8fb] text-[#252733]">
      <div className="min-h-screen">
        {/* HEADER/HERO */}
        <div className="sticky top-0 z-10 bg-white border-b border-[#e7e9ef] min-h-14.5 flex flex-wrap items-center justify-between gap-2 px-4 py-2 sm:px-6">
        ...
        <div className="max-w-425 mx-auto p-4 sm:p-5">
          <div className="flex flex-col gap-4.5 items-start mb-6 lg:flex-row lg:justify-between lg:items-center">
            <div>
              <h1 className="text-[24px] m-0 mb-[4px] font-extrabold text-[#0f172a]">
                Hiệu quả seeding nội bộ
              </h1>
              <p className="m-0 text-[#64748b] text-[14px]">
                Theo dõi thành viên đã nhận bài, đã tương tác, làm thiếu và quá
                hạn.
              </p>
            </div>
            <div className="flex w-full flex-wrap items-center gap-2.5 lg:w-auto lg:gap-3">
              <a
              ...
              >
                <span className="text-base font-bold">+</span> Thêm bài viết
                Seeding
              </button>
            </div>
          </div>
          ...
          {/* MAIN TABS SWITCHER */}
          <div className="flex flex-wrap items-center gap-2 border-b border-gray-200 mb-6 bg-white p-2.5 rounded-2xl shadow-xs">
            <button
              type="button"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5">
                ...
                <div className="border border-rose-100 rounded-2xl p-4 bg-rose-50/20 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                  ...
                  <div className="text-2xl font-black text-rose-950 mt-1">
                    {realStats.activePosts}
                  </div>
                  <span className="text-[11px] text-rose-500 font-medium">
                    Bài viết Seeding hiện tại
                  </span>
                </div>
                <div className="border border-emerald-100 rounded-2xl p-4 bg-emerald-50/20 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                  ...
                  <div className="text-2xl font-black text-emerald-950 mt-1">
                    {realStats.totalCompletedComments} /{" "}
                    {realStats.totalTargetComments}
                  </div>
                  <span className="text-[11px] text-emerald-500 font-medium">
                    {Math.round(
                      (realStats.totalCompletedComments /
                        Math.max(1, realStats.totalTargetComments)) *
                        100,
                    )}
                    % mục tiêu
                  </span>
                ...
                <div className="border border-amber-100 rounded-2xl p-4 bg-amber-50/20 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                  ...
                  <div className="text-2xl font-black text-amber-950 mt-1">
                    {realStats.activeUniqueMembers}
                  </div>
                  <span className="text-[11px] text-amber-500 font-medium">
                    {Math.round(
                      (realStats.activeUniqueMembers /
                        Math.max(1, realStats.totalSeeder)) *
                        100,
                    )}
                    % tổng Seeder
                  </span>
                ...
                <div className="border border-blue-200 rounded-2xl p-4 bg-blue-50/30 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                  ...
                  <div className="text-2xl font-black text-blue-950 mt-1">
                    {realStats.perfectMembers}
                  </div>
                  <span className="text-[11px] text-blue-500 font-medium">
                    Đã xong tất cả bài được giao
                  </span>
                </div>
              </div>
                ...
                <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-bold text-gray-700 uppercase tracking-wider flex items-center gap-1.5">
                      <span>🚩</span> Chiến dịch Seeding đang diễn ra (
                      {campaigns.length})
                    </span>
                  ...
                  <div className="flex items-center gap-2.5 overflow-x-auto pb-1">
                    <button
                      type="button"
                      onClick={() => setSelectedCampaignId("all")}
                      className={`px-3.5 py-2 rounded-xl text-xs font-bold transition shrink-0 border cursor-pointer ${
                        selectedCampaignId === "all"
                    ...
                    {campaigns.map((camp) => {
                      ...
                      return (
                        <div
                          ...
                          className={`px-3.5 py-1.5 rounded-xl text-xs font-bold transition shrink-0 border flex items-center gap-2 ${
                            isSelected
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
                ...
                {activeLeaderboard.length === 0 ? (
                  ...
                  <>
                    {/* Mobile: card list (bảng 7 cột không vừa màn hình nhỏ) */}
                    <div className="flex flex-col gap-2 sm:hidden">
                      {activeLeaderboard.slice(0, 5).map((item, idx) => {
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`shrink-0 inline-flex items-center justify-center px-2.5 py-1 rounded-full text-xs font-bold border ${badgeBg}`}
                            >
                              {rankIcon}
                            </span>
                            ...
                            return (
                              ...
                              >
                                <td className="py-3.5 pl-2">
                                  <span
                                    className={`inline-flex items-center justify-center px-2.5 py-1 rounded-full text-xs font-bold border ${badgeBg}`}
                                  >
                                    {rankIcon}
                                  </span>
                                </td>
                                ...
                                <td className="py-3.5 text-center text-blue-600 font-medium">
                                  {item.total_ontime}
                                </td>
                                <td className="py-3.5 text-center text-gray-700 font-medium">
                                  {item.completion_rate}%
                                </td>
                                <td className="py-3.5 text-right pr-2 font-black text-emerald-600 text-base">
                                  {item.score}
                                </td>
                              </tr>
                            );
              ...
              {canSeeTeamInteractions ? (
                <TeamPerformancePanel email={user?.email} />
              ) : null}
            </div>
          ) : (
            ...
            <div>
              ...
              {selectMode ? (
                <div className="bg-white border border-[#efb2c3] rounded-2xl p-4 mb-4 shadow-[0_1px_3px_rgba(20,25,40,.08)]">
                  ...
                  <label className="text-[12px] font-extrabold block mb-2">
                    Comment hàng loạt:
                  </label>
                  <textarea
                  ...
                  <button
                    ...
                    disabled={
                      isRunning || selectedIds.size === 0 || !isExtensionReady
                    }
                  >
                  ...
                  {runProgress ? (
                    <div className="mt-3 text-[12px] text-[#c71f4d] font-semibold">
                      {runProgress}
                    </div>
                  ) : null}
                </div>
                ...
                <div className="p-3">
                  {isLoading ? (
                    ...
                    <div className="w-full p-12 text-center text-[#64748b] text-sm bg-gray-50/50 rounded-xl my-2 border border-dashed border-gray-200 flex flex-col items-center justify-center gap-2">
                      ...
                      <span className="font-bold text-gray-800 block text-center w-full">
                    ...
                    pagedPosts.map((post) => {
                      ...
                      const campaignName =
                        matchedCamp?.name ||
                        (post as any).campaign_name ||
                        (post as any).campaignName ||
                        null;
                      const campaignColor =
                        matchedCamp?.color_code ||
                        (matchedCamp as any)?.colorCode ||
                        "#fff1f2";
                      const rawTarget =
                        (post as any).target_comments ||
                        (post as any).targetComments;
                      const rawDeadline =
                        (post as any).deadline ||
                        (post as any).due_date ||
                        (post as any).dueDate;
                      ...
                      if (assignedTeams.length === 0) {
                        ...
                        targetTotal = dbTeams
                          .filter(
                            (t) =>
                              assignedTeams.includes(t.id) ||
                              assignedTeams.includes(t.name_team),
                          )
                          .reduce((sum, t) => sum + (t.member_count || 0), 0);
                        if (targetTotal === 0) {
                          targetTotal =
                            Number(rawTarget) > 0 ? Number(rawTarget) : 32;
                        }
                      }
                      ...
                      const progressPercent = Math.min(
                        100,
                        Math.round(
                          (interactedCount / Math.max(1, targetTotal)) * 100,
                        ),
                      );
                      const pendingCount = Math.max(
                        0,
                        targetTotal - interactedCount,
                      );
                      ...
                      const isCompleted =
                        targetTotal > 0 && interactedCount >= targetTotal;
                      const isOverdue =
                        rawDeadline &&
                        new Date(rawDeadline) < new Date() &&
                        !isCompleted;

                      const postUrl =
                        post.permalink_url ||
                        (post as any).link_post ||
                        (post as any).link ||
                        "";
                      ...
                      const isFacebookPost =
                        ...
                        Boolean(
                          ...
                          (postUrl.includes("facebook.com") ||
                            postUrl.includes("fb.watch") ||
                            postUrl.includes("fb.com")),
                        );
                      ...
                      return (
                        <article
                          ...
                          className={`border rounded-2xl mb-4 bg-white transition relative flex flex-col md:flex-row overflow-hidden ${
                            isSelected
                              ? "border-[#c71f4d] bg-[#fff8f9] shadow-md"
                              : "border-[#e7e9ef] hover:border-rose-200 shadow-xs"
                          }`}
                        >
                                ...
                                <div>
                                  <div className="font-bold text-sm text-gray-900">
                                    {post.fanpage_name ||
                                      ...
                                      postUrl.toLowerCase().includes("youtu.be")
                                        ? "Kênh YouTube"
                                        : "Markee Agency")}
                                  </div>
                                  <div className="text-xs text-gray-400 font-medium">
                                    {fmtRelativeTime(post.created_at)} ·{" "}
                                    {fmtDeadline(rawDeadline)}
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="mb-3.5 p-3 border border-dashed border-gray-200 rounded-2xl bg-white flex flex-wrap sm:flex-nowrap items-center justify-between gap-2.5 text-xs">
                                ...
                                <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap text-gray-700">
                                  ...
                                  <div className="flex items-center gap-2.5 text-xs text-gray-500">
                                    ...
                                    <div className="flex items-center gap-1">
                                      <svg
                                        className="w-3.5 h-3.5 text-gray-400 stroke-[1.5]"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                      >
                                      ...
                                      <span className="font-bold text-gray-900">
                                        {formatCompactNumber(
                                          (post as any).public_likes ??
                                            (post as any).fb_total_likes ??
                                            (post as any).likes ??
                                            0,
                                        )}
                                      </span>
                                    ...
                                    <div className="flex items-center gap-1">
                                      ...
                                      >
                                        <path
                                          strokeLinecap="round"
                                          strokeLinejoin="round"
                                          d="M12 20.25c4.97 0 9-3.694 9-8.25s-4.03-8.25-9-8.25S3 7.444 3 12c0 2.104.859 4.023 2.273 5.48.432.447.74 1.04.586 1.641a4.483 4.483 0 0 1-.923 1.785A5.969 5.969 0 0 0 6 21c1.282 0 2.47-.402 3.445-1.087.51-.358 1.155-.41 1.705-.224A8.948 8.948 0 0 0 12 20.25z"
                                        />
                                      </svg>
                                      <span className="font-bold text-gray-900">
                                        {formatCompactNumber(
                                          (post as any).public_comments ??
                                            (post as any).fb_total_comments ??
                                    ...
                                    <div className="flex items-center gap-1">
                                      ...
                                      <span className="font-bold text-gray-900">
                                        {formatCompactNumber(
                                          (post as any).public_shares ??
                                            (post as any).fb_total_shares ??
                                            (post as any).shares_count ??
                                            0,
                                        )}
                                      </span>
                                      <span className="text-gray-400 text-[11px]">
                                ...
                                <div className="flex items-center gap-2 shrink-0 ml-auto sm:ml-0">
                                  <span className="text-gray-400 text-[11px]">
                                    Cập nhật{" "}
                                    {fmtRelativeTime(
                                      (post as any).synced_at ??
                                        (post as any).last_synced_at ??
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      isLinkedInPost
                                        ? handleSyncLinkedInMetrics(post)
                                        : handleSyncPostMetrics(post)
                                    }
                            ...
                            <div className="mb-3.5">
                              <div className="flex justify-between text-xs font-bold mb-1.5">
                                <span className="text-gray-700">
                                  {interactedCount}/{targetTotal} thành viên đã
                                  tương tác
                                </span>
                            ...
                            <div className="grid grid-cols-3 gap-2.5 mb-4">
                              <div className="bg-[#f8fafc] p-3 rounded-xl border border-gray-100">
                                ...
                                <span className="text-sm md:text-base font-bold text-gray-900">
                                  {interactedCount}/{targetTotal}
                                </span>
                              </div>
                              <div className="bg-[#f8fafc] p-3 rounded-xl border border-gray-100">
                                <span className="text-[11px] text-emerald-600 font-semibold block mb-1">
                                  ✓ Hoàn thành
                                </span>
                              ...
                              <div className="bg-[#f8fafc] p-3 rounded-xl border border-gray-100">
                                <span className="text-[11px] text-amber-600 font-semibold block mb-1">
                                  ⏱ Chưa làm
                                </span>
                                  ...
                                  <div className="relative group inline-block">
                                    ...
                                    <div className="absolute bottom-full right-0 mb-1.5 hidden group-hover:block w-max z-10 pointer-events-none">
                                      <div className="bg-gray-900 text-white text-[11px] font-medium rounded-lg py-1 px-2.5 shadow-xl">
                                        Tự động copy mẫu câu &amp; mở bài viết
                                        để Like/Comment/Share để tương tác trên
                                        FB
                                      </div>
                                    </div>
                                  </div>
                ...
                {filteredPosts.length > 0 ? (
                  <div className="flex items-center justify-between px-4 py-3 border-t border-[#e7e9ef]">
                    <div className="text-[12px] text-[#737785]">
                      Trang {currentPage}/{totalPages} — {filteredPosts.length}{" "}
                      bài
                    </div>
          ...
          >
            <div className="w-[min(700px,100%)] bg-white rounded-2xl shadow-[0_30px_80px_rgba(0,0,0,.25)] overflow-hidden">
              <div className="flex justify-between items-center p-4 border-b border-[#e7e9ef]">
                ...
                <button
                  type="button"
                  className="border-0 bg-[#f2f3f6] rounded-lg px-2.5 py-1.75"
                  onClick={closeModal}
                  aria-label="close"
                >
                  ✕
              ...
              <div className="p-5">
                {(() => {
                  ...
                  const ready = modalIsLinkedIn
                    ? isLiExtensionReady
                    : isExtensionReady;
                  return (
                    <div
                      className={`p-3 rounded-xl border text-[12px] mb-4 flex items-center justify-between gap-2 ${
                        ready
                ...
                <div className="text-[13px] text-[#5d616c] mb-3 line-clamp-3">
                  {modalPost.content}
                </div>
                ...
                modalPost.platform !== "linkedin" ? (
                  <div className="mb-3">
                    <label className="text-[12px] font-extrabold block mb-2">
                      Tài khoản Facebook dùng để comment:
                    </label>
                    <select
                    ...
                    >
                      <option value="">
                        -- Tự do (dùng acc đang đăng nhập FB) --
                      </option>
                      {socialAccounts.map((acc) => (
                        <option key={acc.id} value={acc.id}>
                          {acc.account_name}{" "}
                          {acc.account_email ? `(${acc.account_email})` : ""}
                        </option>
                      ))}
                    </select>
                  </div>
                    ...
                    <div className="absolute right-0 top-1 z-20 w-80 max-h-70 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg">
                      {commentTemplateGroups.length === 0 ? (
                        <div className="p-3 text-xs text-slate-500">
                          Chưa có mẫu nào. Vào trang Quick Comment Library để
                          thêm.
                        </div>
                      ) : (
              ...
              <div className="p-4 border-t border-[#e7e9ef] flex justify-end gap-2">
                <button
                  type="button"
                  className="border border-[#e7e9ef] bg-white rounded-xl px-4 py-2 font-extrabold"
                  onClick={closeModal}
                  disabled={isRunning}
                >
                  Để sau
              ...
              <div className="flex justify-between items-center p-5 border-b border-gray-100">
                <div>
                  <h3 className="text-base font-bold text-gray-900">
                    Chi tiết tương tác thành viên
                  </h3>
                  <div className="text-xs text-gray-500 mt-0.5 line-clamp-1">
                    Bài: {interactionsPost.content || "(Bài viết Facebook)"}
                  </div>
                </div>
                <button
                  type="button"
              ...
              <div className="p-4 bg-gray-50/70 border-b border-gray-100 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <select
                    ...
                    onChange={(e) =>
                      setInteractionsStatusFilter(e.target.value)
                    }
                  ...
                  >
                    ...
                    {dbTeams.length > 0
                      ...
                      : ["Sale", "Marketing", "Presale", "Operation"].map(
                          (t) => (
              ...
              <div className="p-4 overflow-y-auto flex-1">
                {isLoadingInteractions ? (
                  <div className="p-12 text-center text-gray-500 text-xs">
                    Đang tải chi tiết tương tác thành viên...
                  </div>
                ) : (
                      ...
                      <tbody className="divide-y divide-gray-50 text-gray-700 font-medium">
                        {(() => {
                          ...
                          const filteredRoster = (
                            interactionsItems as any[]
                          ).filter((m) => {
                            // Lọc theo trạng thái
                            if (
                              interactionsStatusFilter !== "all" &&
                              m.status !== interactionsStatusFilter
                            ) {
                              return false;
                            }
                            // Lọc theo Team
                            if (interactionsTeamFilter !== "all") {
                              const matchedTeamObj = dbTeams.find(
                                (t) => t.id === interactionsTeamFilter,
                              );
                              const targetTeamName =
                                matchedTeamObj?.name_team ||
                                interactionsTeamFilter;
                              if (
                                (m.team || "").toLowerCase() !==
                          ...
                          filteredRoster.sort((a, b) => {
                            ...
                            if (
                              ...
                              const aTime = a.raw_created_at
                                ? new Date(a.raw_created_at).getTime()
                                : Infinity;
                              const bTime = b.raw_created_at
                                ? new Date(b.raw_created_at).getTime()
                                : Infinity;
                              return aTime - bTime;
                            }
                            return 0;
                              ...
                              <td className="py-3 text-center">
                                {m.like ? (
                                  <span
                                    ...
                                    title={
                                      m.like_time
                                        ? `Đã like lúc: ${formatMemberTime(m.like_time)}`
                                        : "Đã like"
                                    }
                                  >
                                    ✓
                              ...
                              <td className="py-3 text-center">
                                {m.comment ? (
                                  <span
                                    ...
                                    title={
                                      m.comment_time
                                        ? `Đã comment lúc: ${formatMemberTime(m.comment_time)}`
                                        : "Đã comment"
                                    }
                                  >
                                    ✓
                              ...
                              <td className="py-3 text-center">
                                {m.share ? (
                                  <span
                                    ...
                                    title={
                                      m.share_time
                                        ? `Đã share lúc: ${formatMemberTime(m.share_time)}`
                                        : "Đã share"
                                    }
                                  >
                                    ✓
              ...
              <div className="flex justify-between items-center p-4 border-b border-[#e7e9ef]">
                <div>
                  <b className="text-base text-gray-900">
                    Sửa bài viết & Gia hạn Target KPI
                  </b>
                  <div className="text-[12px] text-gray-500 mt-0.5">
                    Cập nhật deadline, chiến dịch và chỉ tiêu tương tác cho bài
                    viết.
                  </div>
                </div>
                <button
              ...
              <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2.5 flex-wrap pt-1">
                    {dbTeams.length > 0
                      ? dbTeams.map((team) => {
                          const isChecked =
                            editAssignedTeams.includes(team.id) ||
                            editAssignedTeams.includes(team.name_team);
                          return (
                            <label
                              key={team.id}
                              className={`flex items-center gap-1.5 text-xs font-medium cursor-pointer border px-3 py-1.5 rounded-xl transition ${
                                isChecked
                            ...
                            >
                              ...
                              />
                              <span>
                                {team.name_team} ({team.member_count})
                              </span>
                            </label>
                          );
                        })
                ...
                <div className="p-3.5 bg-rose-50/40 rounded-xl border border-rose-100">
                  <span className="text-xs font-bold text-rose-800 block mb-2">
                    💬 Target KPI Comment Bài Viết:
                  </span>
                  <div>
                    <label className="text-[11px] font-semibold text-gray-600 block mb-1">
                      Target Comment (Mục tiêu số lượt Comment):
                    </label>
                    <input
                      type="number"
                      value={editTargetComments}
                      onChange={(e) =>
                        setEditTargetComments(Number(e.target.value))
                      }
                    ...
                    {[
                      ...
                      <button
                        ...
                        className={`w-8 h-8 rounded-full border-2 transition ${
                          newCampaignColor === c
                            ? "border-[#be123c] scale-110 shadow-sm"
                            : "border-gray-200"
                        }`}
                      />
                    ))}
            ...
            <div className="w-[min(420px,100%)] bg-white rounded-2xl shadow-[0_30px_80px_rgba(0,0,0,.25)] overflow-hidden p-6 text-center">
              ...
              <p className="text-xs text-gray-500 mb-6 leading-relaxed">
                ...
                <b className="text-gray-900">
                  &quot;{deletingCampaignTarget.name}&quot;
                </b>{" "}
                không? Các bài viết thuộc chiến dịch này sẽ không bị xóa.
              </p>
              ...
              <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
                ...
                {(() => {
                  ...
                  return (
                    <>
                      {isLinkedInTaskLink &&
                      liAutoFetchStatus === "fetching" ? (
                        <p className="text-[11px] text-[#0a66c2] font-semibold">
                          ⏳ Đang tự động lấy nội dung bài viết LinkedIn...
                        </p>
                      ) : null}

                      {isLinkedInTaskLink && liAutoFetchStatus === "success" ? (
                        <p className="text-[11px] text-green-700 font-semibold">
                          ✓ Đã tự động lấy nội dung bài viết LinkedIn.
                        </p>
                      ) : null}
                    </>
                  );
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2.5 flex-wrap pt-1">
                    {dbTeams.length > 0
                      ? dbTeams.map((team) => {
                          const isChecked =
                            taskAssignedTeams.includes(team.id) ||
                            taskAssignedTeams.includes(team.name_team);
                          return (
                            <label
                              key={team.id}
                              className={`flex items-center gap-1.5 text-xs font-medium cursor-pointer border px-3 py-1.5 rounded-xl transition ${
                ...
                <div className="p-3.5 bg-rose-50/50 rounded-xl border border-rose-100">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setTaskTargetComments(Number(e.target.value))
                      }
                      disabled={isSubmittingTask}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none bg-white font-bold text-gray-900 focus:border-rose-500"
                    />
                    ...
                    ) : taskTargetComments <= 0 ||
                      taskTargetComments > totalSelectedMembers ? (
                      <p className="text-[11px] font-semibold text-red-600 mt-1 flex items-center gap-1">
                        ⚠️ Target Comment phải từ 1 đến tối đa{" "}
                        {totalSelectedMembers}.
                      </p>
                    ) : null}
                  </div>
            ...
            <div className="w-[min(800px,100%)] bg-white rounded-2xl shadow-[0_30px_80px_rgba(0,0,0,.25)] overflow-hidden flex flex-col max-h-[85vh]">
              ...
              <div className="p-4 bg-gray-50 border-t border-gray-100 flex flex-wrap items-center justify-between gap-2">
                <span className="text-xs text-gray-500 font-semibold">
                  Trang {leaderboardCurrentPage} /{" "}
                  {Math.max(1, leaderboardTotalPages)} (
                  {activeLeaderboard.length} thành viên)
                </span>
                <div className="flex gap-2">
                  <button
                    type="button"
                    ...
                    onClick={() =>
                      setLeaderboardCurrentPage((p) => Math.max(1, p - 1))
                    }
        ...
        {toast ? (
          <div
            className={`fixed left-4 right-4 bottom-4 sm:left-auto sm:right-6 sm:bottom-6 z-[50] flex items-center gap-3 px-4 py-3 bg-white text-[#1f2937] text-[13px] font-semibold rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.12)] transition-all animate-in fade-in slide-in-from-bottom-2 sm:max-w-sm ${
              toast.type === "error"
