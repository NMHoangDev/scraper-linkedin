...
export function BulkCommentLauncher({
  posts,
  onComplete,
}: BulkCommentLauncherProps) {
    ...
    const handleMessage = async (event: MessageEvent) => {
      if (event.data?.action === "COMMENT_EXTENSION_READY") {
        ...
        if (pendingIds.length > 0) {
          ...
          setProgress((p) =>
            p ? { ...p, status: "Đã hoàn tất comment hẹn giờ!" } : null,
          );
        } else {
          setProgress((p) =>
            p ? { ...p, status: "Hoàn tất toàn bộ tiến trình!" } : null,
          );
          if (onComplete) onComplete(Array.from(selectedUrls));
        }
      } else if (event.data?.action === "STATUS_RESPONSE") {
        const { isCommenting: extIsCommenting, currentProgress } =
          event.data.payload || {};
        if (extIsCommenting) {
          setIsCommenting(true);
          setIsExpanded(true);
  ...
  useEffect(() => {
    ...
    const pollScheduled = async () => {
      ...
      try {
        ...
        const now = new Date();
        const dueComments = res.data.filter(
          (sc) =>
  ...
  const handleSchedule = async () => {
    ...
    if (new Date(scheduledAt) <= new Date())
      return alert("Thời gian phải trong tương lai!");
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden flex flex-col transition-all duration-300 w-full mb-6 relative">
      {/* Header */}
      <div className="flex flex-col items-start gap-3 p-4 bg-slate-50/50 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3 min-w-0 w-full sm:w-auto">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-500 leading-tight mt-0.5">
              Tự động comment chạy ngầm trên trình duyệt. Không làm gián đoạn
              công việc của bạn.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-[#DC2626] hover:bg-[#B91C1C] text-white text-xs font-bold transition-all shadow-sm hover:shadow active:scale-[0.98] flex items-center justify-center gap-1.5 cursor-pointer flex-1 sm:flex-none"
          >
            <span className="material-symbols-outlined text-[16px]">
      ...
      {isExpanded && (
        <div className="p-4 border-t border-slate-100 flex flex-col gap-5 bg-white">
          <div
            className={`p-3 rounded-xl border text-sm flex items-center gap-2 ${isReady ? "bg-green-50 border-green-200 text-green-700" : "bg-amber-50 border-amber-200 text-amber-700"}`}
          >
            <span className="material-symbols-outlined text-[18px]">
              {isReady ? "check_circle" : "warning"}
            </span>
            {isReady
              ? "Extension đã sẵn sàng chạy ngầm"
              : "Đang chờ kết nối Extension. Vui lòng cài đặt và F5 lại trang."}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-4">
              ...
              <div className="space-y-2 flex-1 flex flex-col relative">
                ...
                {isTemplatePickerOpen && (
                  <div className="absolute right-0 top-8 z-20 w-80 max-w-[85vw] max-h-[320px] overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg">
            ...
            <div className="space-y-2 flex flex-col">
              <div className="flex items-center justify-between">
                ...
                >
                  {selectedUrls.size === validPosts.length
                    ? "Bỏ chọn tất cả"
                    : "Chọn tất cả"}
                </button>
              </div>
                ...
                {validPosts.length === 0 ? (
                  ...
                  validPosts.map((post) => {
                    ...
                    return (
                      <label
                        key={post.id}
                        className={`flex items-start gap-3 p-3 cursor-pointer hover:bg-slate-50 transition ${isCommenting ? "opacity-70 pointer-events-none" : ""}`}
                      >
                        <input
                          type="checkbox"
          ...
          <div className="flex flex-col gap-3 pt-3 border-t border-slate-100">
            {progress && (
              <div className="bg-red-50 border border-red-100 rounded-xl p-3 space-y-2">
                <div className="flex justify-between text-xs text-red-800 font-bold">
                  <span>
                    Tiến trình: {progress.current} / {progress.total}
                  </span>
                  <span>
            ...
            {scheduleResult && (
              <div
                className={`rounded-xl p-3 text-sm font-medium ${scheduleResult.failed === 0 ? "bg-emerald-50 border border-emerald-200 text-emerald-700" : "bg-amber-50 border border-amber-200 text-amber-700"}`}
              >
              ...
              {isCommenting && (
                <div className="flex items-center gap-2 mr-auto">
                  <div className="flex items-center text-xs font-medium text-slate-500">
                    <span className="material-symbols-outlined animate-spin text-[16px] mr-1">
                      progress_activity
                    </span>
                    {progress
