  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 bg-[#f8f6f2] min-h-screen">
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        <span className="text-xs text-on-surface-variant bg-surface-container-highest/60 px-3 py-1 rounded-full font-medium border border-outline-variant">
          {accounts.length} tài khoản
        </span>

        <div className="ml-auto flex items-center gap-3 flex-wrap">
          <button className="flex items-center gap-1.5 text-xs font-semibold px-4 py-2 rounded-xl bg-surface border border-outline-variant hover:bg-surface-container-low text-on-surface shadow-sm transition-all">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-surface text-on-surface placeholder:text-on-surface-variant text-sm pl-9 pr-4 py-2 rounded-xl border border-outline-variant focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 w-56 shadow-sm transition-all"
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mt-6 mb-5">
        <h3 className="text-sm font-bold text-on-surface">Tài khoản</h3>
        <span className="text-xs text-on-surface-variant bg-surface-container-highest/60 px-2 py-0.5 rounded-full font-medium border border-outline-variant">{accounts.length}</span>
        <div className="flex-1 border-t border-outline-variant" />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {filtered.map((acc) => (
          <div key={acc.id} className="bg-[#f2ede6] rounded-xl p-4 shadow-sm border border-[#e8e2da] hover:shadow-md transition-shadow relative group">
            <div className="flex items-start gap-4 mb-4">
              <div className="relative">
                <div className="w-12 h-12 bg-surface-container-highest rounded-full flex items-center justify-center text-xl font-bold text-on-surface-variant overflow-hidden border-2 border-white shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="font-bold text-on-surface truncate text-base">{acc.name || acc.email.split('@')[0]}</h4>
                <p className="text-xs text-on-surface-variant truncate mt-0.5 font-medium">{acc.email}</p>
                {acc.phone && (
              ...
              <button className="text-on-surface-variant hover:text-on-surface-variant p-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex-1 bg-[#e4dfd8] hover:bg-[#d8d3cc] text-on-surface font-semibold py-2 rounded-xl text-sm transition-colors border border-[#d5d0c9]"
              >
                Ngắt kết nối
              </button>
            </div>
