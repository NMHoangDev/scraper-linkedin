    ...
    <div className="flex flex-col gap-5">
      ...
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col">
            <h2 className="text-lg font-bold text-slate-800 leading-tight">Quản lý tài khoản</h2>
            <p className="text-[11px] text-slate-500 leading-tight">Theo dõi trạng thái Zalo, share inbox với leader</p>
          </div>
          <span className="ml-1 rounded-full bg-slate-100 border border-slate-200 px-2.5 py-0.5 text-[11px] font-bold text-slate-600">
      ...
      {flow.userId && flow.userId !== "default" && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-green-50/60 border border-emerald-200/60 rounded-lg px-3 py-2.5">
            ...
            <p className="text-xl font-bold text-slate-800 mt-1.5 leading-none">
          ...
          <div className="bg-gradient-to-br from-violet-50 to-purple-50/60 border border-violet-200/60 rounded-lg px-3 py-2.5">
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">đang lắng nghe</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 px-4 py-3 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1">
            <MaterialIcon name="call" className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400 text-[14px]" />
            <input
        ...
        {filteredAccounts.map((account) => {
          ...
          return (
            <div
              ...
              className="bg-white group relative flex flex-col rounded-xl border border-slate-200 p-3.5 shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 hover:border-[#E3000F]/40"
            >
              {/* Compact Header: Avatar + Name + Status + 3-dot menu */}
              <div className="flex items-start gap-2.5">
                <div className="relative shrink-0">
                  <div className="h-11 w-11 rounded-lg bg-gradient-to-br from-red-50 to-red-100/60 flex items-center justify-center text-[15px] font-bold text-[#E3000F] border border-red-100">
                    {account.label?.[0]?.toUpperCase() || "Z"}
                  </div>
                  <div className="absolute -bottom-0.5 -right-0.5 border-[2px] border-white rounded-full bg-white">
                ...
                <div className="flex-1 min-w-0 -mt-0.5">
                  <div className="flex items-start justify-between gap-1">
                    <div className="text-[13.5px] font-bold text-slate-800 truncate group-hover:text-[#E3000F] transition-colors leading-tight">
                    ...
                    <div className="relative shrink-0 -mr-1">
                      <button
                        ...
                        className="text-slate-400 hover:text-slate-700 h-6 w-6 rounded-md hover:bg-slate-100 transition flex items-center justify-center"
                        title="Tùy chọn"
                      >
                      ...
                      {activeMenuAccountId === account.account_id && (
                        <>
                          ...
                          />
                          <div className="absolute right-0 mt-1 w-44 bg-white border border-slate-200 rounded-lg shadow-xl py-1 z-20 overflow-hidden">
                            <button
                              onClick={() => {
                              ...
                              className="w-full text-left px-3 py-2 text-[12.5px] hover:bg-slate-50 flex items-center gap-2 font-semibold text-slate-700 transition-colors"
                            >
                              <MaterialIcon name="edit" className="text-[14px] text-[#E3000F]" />
                              Chỉnh sửa
                            </button>
                            <button
                              onClick={() => {
                              ...
                              className="w-full text-left px-3 py-2 text-[12.5px] hover:bg-red-50 flex items-center gap-2 font-semibold text-red-600 border-t border-slate-100 transition-colors"
                            >
                  ...
                  {/* Status badge */}
                  <div className="mt-1">
                    <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold tracking-wide ${
              ...
              <div className="mt-3 flex flex-col gap-1.5">
                <div className="flex items-center gap-1.5 text-[12px] text-slate-600">
                  <MaterialIcon name="call" className="text-[13px] text-slate-400 shrink-0" />
                  {account.phone ? (
                ...
                <div className="flex items-center gap-1.5 text-[12px] text-slate-600">
                  <MaterialIcon name="key" className="text-[13px] text-slate-400 shrink-0" />
                  <span className="font-mono text-[11px] bg-slate-100 px-1.5 py-0.5 rounded text-slate-500 truncate">
      ...
      {filteredAccounts.length === 0 && !flow.isLoadingAccounts && (
        <div className="border border-dashed border-slate-300 bg-slate-50/50 rounded-xl p-8 text-center">
          <div className="h-12 w-12 mx-auto rounded-full bg-slate-100 flex items-center justify-center mb-2">
            <MaterialIcon name="account_circle" className="text-slate-400 text-[24px]" />
          </div>
      ...
      {editingAccount && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white border border-slate-200 rounded-2xl p-5 w-[360px] shadow-2xl flex flex-col gap-4">
            <div className="flex items-center gap-2.5 border-b border-slate-100 pb-3">
              <div className="h-8 w-8 rounded-lg bg-red-50 flex items-center justify-center">
                <MaterialIcon name="edit" className="text-[#E3000F] text-[16px]" />
              </div>
              <div className="flex-1">
                <h3 className="text-[14px] font-bold text-slate-800 leading-tight">Chỉnh sửa tài khoản</h3>
                <p className="text-[11px] text-slate-500 leading-tight">Cập nhật tên & số điện thoại</p>
              </div>
              <button
                onClick={() => setEditingAccount(null)}
                className="text-slate-400 hover:text-slate-700 h-7 w-7 rounded-md hover:bg-slate-100 transition flex items-center justify-center"
              >
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[11.5px] font-semibold text-slate-600">Tên tài khoản</label>
              <input
                type="text"
                value={editLabel}
                onChange={(e) => setEditLabel(e.target.value)}
                className="border border-slate-200 bg-white h-9 rounded-lg px-3 text-[13px] focus:border-[#E3000F] focus:ring-2 focus:ring-[#E3000F]/20 focus:outline-none transition-all"
                placeholder="Vd: Việt, Nam..."
              />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-[11.5px] font-semibold text-slate-600">Số điện thoại</label>
              <input
                type="text"
                value={editPhone}
            ...
            <div className="flex justify-end gap-2 mt-1 border-t border-slate-100 pt-3">
              <button
                type="button"
                onClick={() => setEditingAccount(null)}
                className="h-9 border border-slate-200 bg-white text-slate-700 px-4 rounded-lg text-[12.5px] font-semibold hover:bg-slate-50 transition"
              >
                Hủy
              </button>
              <button
                ...
                className="h-9 bg-[#E3000F] text-white px-5 rounded-lg text-[12.5px] font-semibold hover:bg-red-700 transition shadow-sm shadow-red-500/20 disabled:opacity-50 disabled:shadow-none"
              >
