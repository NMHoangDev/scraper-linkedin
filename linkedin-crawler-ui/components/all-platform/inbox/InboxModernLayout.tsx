...
function displayMessage(message: Msg): { content: string; time: string } {
  const matched = message.text.match(
    /^Tin nhắn do .+? gửi lúc (.+?):\s*([\s\S]*)$/i,
  );
  let content = matched ? (matched[2] || "").trim() : message.text;
  const time = matched ? (matched[1] || "").trim() : message.time || "";
  const stripped = content
    .replace(
      /^(?:(?:Thứ\s+\S+\s+)?\d{1,2}(?::\d{2})?(?:sáng|chiều|ch|sa|CH|SA|AM|PM)?)\s*[:\n]+\s*/i,
      "",
    )
    .trim();
...
function getWeekNumber(date: Date): number {
  ...
  const diffDays = Math.round(
    (date.getTime() - startMonday.getTime()) / 86400000,
  );
...
export default function InboxModernLayout(props: Props) {
  ...
  const [templateGroupId, setTemplateGroupId] = useState(
    QUICK_REPLY_GROUPS[0].id,
  );
  const [kpiToast, setKpiToast] = useState<{ msg: string; ok: boolean } | null>(
    null,
  );
  const [noteDraftState, setNoteDraftState] = useState({
    convId: "",
    value: "",
  });
  ...
  const [trackSelectedIds, setTrackSelectedIds] = useState<Set<string>>(
    new Set(),
  );
  const selectedSession = sessions.find((s) => s.user_id === acc);
  const selectedConv =
    activeConvs.find((c) => c.conv_id === openConv) ||
    filtered.find((c) => c.conv_id === openConv);
  const selectedArchive = archives.find((a) => a.conv_id === openConv);
  const selectedName = archiveReading
    ? selectedArchive?.name
    : selectedConv?.name;
  const selectedPreview = archiveReading
    ? selectedArchive?.preview
    : selectedConv?.preview;
  const selectedNote = openConv
    ? (customerNotes[openConv] ?? selectedArchive?.note ?? "")
    : "";
  const activeTemplateGroup =
    QUICK_REPLY_GROUPS.find((g) => g.id === templateGroupId) ||
    QUICK_REPLY_GROUPS[0];
  const noteDraft =
    noteDraftState.convId === openConv ? noteDraftState.value : selectedNote;
  const setNoteDraft = (value: string) =>
    setNoteDraftState({ convId: openConv, value });
  ...
  const trackSearchResults = useMemo(() => {
    ...
    const filtered = q
      ? trackSearchPool.filter((c) => (c.name || "").toLowerCase().includes(q))
      : trackSearchPool;
    return filtered.slice(0, TRACK_SEARCH_LIMIT);
  }, [trackSearchQuery, trackSearchPool]);

  const canSend =
    !!openConv && !archiveReading && accOnline && !accPaused && !needRelogin;
  ...
  // Cac canh bao trang thai (mat ket noi/cookie het han/can PIN/thieu extension):
  // sticky ngay duoi header, rong theo cot noi dung — xem comment o JSX ben duoi.
  // Origin hien tai (vd http://localhost:8080) — dung de chan doan nhanh khi
  // backend/extension dang tro sai cong (xem NEXT_PUBLIC_LINKEDIN_CRAWLER_API_URL
  // trong .env va whitelist domain trong manifest extension Markee).
  const currentOrigin =
  ...
  if (connErr)
    statusAlerts.push({
      key: "connErr",
      tone: "amber",
      icon: <TriangleAlert size={16} className="mt-0.5 shrink-0" />,
      text: (
        <>
          Không kết nối được Facebook automation service. Kiểm tra biến{" "}
          ...
          — hiện đang gọi API từ <b>{currentOrigin || "trang này"}</b>, phải trỏ
          đúng sang cổng backend đang chạy (mặc định{" "}
          <code className="rounded bg-black/5 px-1 py-0.5 text-[11px]">
            http://localhost:8000
          </code>
          ), không phải trỏ về chính cổng của dashboard.
        </>
      ),
    });
  if (needRelogin)
    statusAlerts.push({
      key: "needRelogin",
      tone: "red",
      icon: <Lock size={16} className="mt-0.5 shrink-0" />,
      text: (
        <>
          Cookie hết hạn. Vào Quản lý tài khoản &gt; Tài khoản FB &amp; KPI để
          đăng nhập lại.
        </>
      ),
    });
  if (needsPin)
    statusAlerts.push({
      ...
      text:
        needsPinMessage ||
        "Facebook yêu cầu nhập mã PIN thủ công để khôi phục 1 đoạn chat. Vào Messenger trên máy nhân viên, mở hội thoại, nhập mã PIN 1 lần rồi thử lại.",
    });
  if (
    role === "member" &&
    extInstalled !== null &&
    !sessions.some((s) => s.owner === owner)
  ) {
    statusAlerts.push({
      key: "extMissing",
      tone: "blue",
      ...
      text:
        extInstalled === false ? (
          <>
            Chưa cài hoặc chưa mở extension Markee — hoặc extension đã cài nhưng
            chưa được whitelist cho <b>{currentOrigin || "domain này"}</b>. Kiểm
            tra manifest extension (content_scripts.matches /
            externally_connectable) đã thêm đúng domain dashboard đang chạy
            chưa, rồi tải lại trang.
          </>
        ) : (
          "Extension sẵn sàng nhưng tài khoản FB chưa kết nối. Mở extension, đăng nhập Facebook và giữ 1 tab Messenger đang mở."
        ),
    });
  }
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden text-foreground">
      {/* Canh bao trang thai (mat ket noi/cookie het han/can PIN/thieu extension).
          Truoc day dung "fixed" + "100vw" de xep chong giua man hinh — de vo layout
          khi co ancestor bi transform (thu nho containing block), lam khung canh bao
          bi bop hep, chu bi ngat dong tung tu. Gio doi ve trong luong trang binh
          thuong (sticky ngay duoi thanh tieu de), rong 100% cot noi dung -> luon
          hien dung, khong con lap len card KPI hay bi cat chu. */}
      {statusAlerts.length > 0 && (
        <div className="sticky top-0 z-30 -mx-1 mb-4 flex flex-col gap-2 bg-background/95 px-1 pt-1 pb-1 backdrop-blur-sm sm:top-2">
          {statusAlerts.map((a) => (
            <div
              key={a.key}
              className={cn(
                "flex w-full min-w-0 items-start gap-2 rounded-xl border px-3 py-2.5 text-xs font-semibold leading-5 shadow-sm sm:px-4 sm:py-3 sm:text-sm",
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard
          ...
          hint={`${stats.pushed} đẩy Zalo`}
          iconClassName="bg-emerald-100 text-emerald-600"
        />
        <StatCard
          icon={CheckCircle2}
          label="Online"
          value={sessions.filter((s) => s.status === "online").length}
          hint={`${sessions.length} tài khoản`}
      ...
      <div className="mb-4 rounded-xl border border-border bg-card">
            ...
            <div>
              ...
              <div className="mt-0.5 flex items-center gap-2 text-sm font-semibold text-foreground">
                <span
                  className={cn(
                    "size-2 rounded-full",
                    statusDotClass(selectedSession?.status),
                  )}
                />
                {selectedSession
                  ? accLabel(selectedSession)
                  : "Chưa chọn tài khoản"}
                {selectedSession && (
              ...
              {accountBarOpen && (
                <>
                  ...
                  {(role === "admin" || role === "leader") &&
                    typeof toggleExtraAccount === "function" && (
                        ...
                        {showAddAccountPicker && (
                          <div className="absolute right-0 z-20 mt-1 max-h-80 w-72 overflow-auto rounded-xl border border-border bg-popover p-2 shadow-lg">
                            <div className="px-1 pb-1.5 text-[11px] text-muted-foreground">
                              Chọn thêm acc ngoài phạm vi mặc định (team/của
                              bạn) để hiện trong Inbox — chỉ lưu trên trình
                              duyệt này.
                            </div>
                            {rawSessions.length === 0 ? (
                              ...
                              rawSessions.map((s) => {
                                const inDefaultScope =
                                  allowedOwnerIds === null ||
                                  (!!s.owner &&
                                    allowedOwnerIds.has(String(s.owner)));
                                const checked = extraAccountIds.has(s.user_id);
                                return (
                                  <label
                                    key={s.user_id}
                                    className={cn(
                                      "flex items-center gap-2 rounded-md px-1.5 py-1 text-xs cursor-pointer hover:bg-accent",
                                      inDefaultScope && "opacity-50",
                                    )}
                                  >
                                    <input
                                      type="checkbox"
                                      checked={checked || inDefaultScope}
                                      disabled={inDefaultScope}
                                      onChange={() =>
                                        toggleExtraAccount(s.user_id)
                                      }
                                    />
                                    <span
                                      className={cn(
                  ...
                  <div className="hidden text-xs text-muted-foreground md:block">
                    Online realtime, offline chỉ xem dữ liệu.
                  </div>
                </>
              )}
              <button
          ...
          {accountBarOpen &&
            (sessions.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border bg-muted/40 px-3 py-4 text-sm text-muted-foreground">
                {extInstalled === false
                  ? "Chưa thấy extension. Hãy cài và mở extension."
        ...
        {(accountOwnerEmail || userEmail) && (
          <div className="grid grid-cols-1 gap-4 border-t border-border p-4 xl:grid-cols-2">
            <KpiProgressCard
              email={accountOwnerEmail || userEmail}
              type="inbox"
            />
            <KpiProgressCard
              email={accountOwnerEmail || userEmail}
      ...
      <div className="grid min-w-0 grid-cols-1 gap-4 lg:grid-cols-[280px_minmax(0,1fr)_340px]">
        {/* Cot 1: Danh sach hoi thoai */}
        <section className="flex min-h-[520px] h-[656px] min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-card lg:h-[656px] xl:h-[716px]">
            ...
            {trackSearchOpen && (
              <div className="mb-2 rounded-xl border border-border bg-muted/40 p-2">
                <input
                  ...
                  placeholder={
                    loadingAllConvs
                      ? "Đang tải toàn bộ hội thoại..."
                      : "Lọc theo tên (để trống = xem hết hội thoại cũ)..."
                  }
                ...
                <div className="mt-2 flex items-center justify-between gap-2">
                  ...
                  >
                    ...
                    trackSelectedIds.size === trackSearchResults.length
                      ? "Bỏ chọn tất cả"
                  ...
                  {trackSelectedIds.size > 0 && (
                    <button
                      type="button"
                      onClick={() => {
                        trackSelectedIds.forEach((id) =>
                          mark(id, "is_customer", true),
                        );
                ...
                <div className="mt-2 max-h-64 space-y-1 overflow-auto">
                  {loadingAllConvs ? (
                    ...
                    <div className="px-1 py-2 text-xs text-muted-foreground">
                      Không có hội thoại nào khớp (chỉ tìm được trong dữ liệu đã
                      quét trước đó).
                    </div>
                  ) : (
                        ...
                        >
                          <input
                            ...
                            onChange={() =>
                              setTrackSelectedIds((prev) => {
                                ...
                                if (next.has(c.conv_id)) next.delete(c.conv_id);
                                else next.add(c.conv_id);
                                return next;
                              })
                            }
                          />
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="truncate text-xs text-muted-foreground">
                              {c.preview || "(không có preview)"} · {c.time}
                            </div>
                          </div>
                          <button
                            type="button"
                  ...
                  {trackSearchPool.length > TRACK_SEARCH_LIMIT && (
                    <div className="px-1 pt-1 text-[10px] text-muted-foreground">
                      Chỉ hiện {TRACK_SEARCH_LIMIT}/{trackSearchPool.length} —
                      gõ tên để lọc bớt.
                    </div>
                  )}
                </div>
                ...
                archives.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "mb-0.5 flex w-full items-center gap-3 rounded-lg px-2 py-2.5 text-left transition",
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-baseline justify-between gap-2">
                          <div className="truncate text-sm font-semibold text-foreground">
                            {item.name || item.conv_id}
                          </div>
                          <span className="shrink-0 text-[11px] text-muted-foreground">
                            {item.messages_count || 0} tin
                          </span>
                        </div>
                        <div className="truncate text-xs text-muted-foreground">
                          {item.preview || "—"}
                        ...
                        {(item.is_customer || item.pushed_to_zalo) && (
                          <div className="mt-1 flex flex-wrap gap-1">
                            {item.is_customer && (
                              <Badge
                                variant="secondary"
                                className="border-transparent bg-emerald-100 text-[9px] text-emerald-700"
                              >
                                khách
                              </Badge>
                            )}
                            {item.pushed_to_zalo && (
                              <Badge
                                variant="secondary"
                                className="border-transparent bg-blue-100 text-[9px] text-blue-700"
                              >
                                Zalo
                              </Badge>
                            )}
                      ...
                      {(needsReply(conv) ||
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {needsReply(conv) && (
                            <Badge
                              variant="secondary"
                              className="border-transparent bg-red-100 text-[9px] text-red-700"
                            >
                              cần trả lời
                            </Badge>
                          )}
        ...
        <section className="flex min-h-[520px] h-[656px] min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-card lg:h-[656px] xl:h-[716px]">
            ...
            {selectedConv && !archiveReading && (
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={() =>
                    mark(
                      selectedConv.conv_id,
                      "is_customer",
                      !selectedConv.is_customer,
                    )
                  }
                  className="rounded-lg border border-border px-2.5 py-1.5 text-xs font-semibold transition hover:border-primary"
                >
                ...
                <button
                  onClick={() => saveArchive(selectedConv.conv_id, false)}
                  className="rounded-lg border border-border px-2.5 py-1.5 text-xs font-semibold transition hover:border-primary"
                >
                  Lưu
                </button>
                <button
                  onClick={() => confirmHide(selectedConv)}
                  className="rounded-lg border border-destructive/30 px-2.5 py-1.5 text-xs font-semibold text-destructive transition hover:bg-destructive/10"
                >
                  Ẩn
                </button>
              </div>
          ...
          >
            {!openConv ? (
              ...
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                {archiveReading
                  ? "Bản lưu chưa có nội dung."
                  : needRelogin
                    ? "Cookie hết hạn."
                    : accPaused
                      ? "Realtime đang tạm dừng."
                      : !accOnline
                        ? "Tài khoản offline."
              ...
              <div className="space-y-3">
                {msgs.map((message, index) => {
                  ...
                  return (
                    <div
                      key={`${index}-${message.clientId || time}`}
                      className={cn(
                        "flex",
        ...
        <aside className="flex min-h-[520px] h-[656px] min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-card lg:h-[656px] xl:h-[716px]">
          <div className="grid shrink-0 grid-cols-3 border-b border-border bg-muted/40 text-sm font-bold">
            <button
              ...
              className={cn(
                "flex items-center justify-center gap-1.5 border-b-2 py-3 text-xs transition",
                panelTab === "templates"
                  ...
                  <div className="space-y-3">
                    ...
                    {selectedConv && !archiveReading && (
                      <div className="grid gap-2">
                        ...
                        <button
                          onClick={() =>
                            saveArchive(selectedConv.conv_id, false)
                          }
                          className="rounded-xl border border-border px-3 py-2 text-sm font-semibold transition hover:border-primary"
                        >
                          Lưu khách
              ...
              <div>
                ...
                {(role === "admin" || role === "leader") &&
                  (() => {
                    ...
                    const confirmedWeekNumber = confirmedAt
                      ? getWeekNumber(new Date(confirmedAt))
                      : getCurrentWeekNumber();
                    return (
                      <div
                        className={cn(
                          "mb-5 rounded-xl border p-4 transition-colors",
                          isConfirmed
                      ...
                      >
                        <div className="mb-1 flex flex-wrap items-center justify-between gap-1.5">
                          <div
                            className={cn(
                              "flex min-w-0 flex-wrap items-center gap-1.5 text-xs font-bold uppercase",
                              isConfirmed
                                ? "text-emerald-700"
                                : "text-foreground",
                            )}
                          >
                            ...
                            <span className="shrink-0 whitespace-nowrap rounded-full bg-black/5 px-1.5 py-0.5 text-[10px] font-semibold normal-case text-muted-foreground">
                              Tuần {confirmedWeekNumber}
                            </span>
                          </div>
                        ...
                        {openConv &&
                          (inboxKpiWeekDates[openConv] ? (
                            <div className="mb-3 rounded-lg bg-violet-50 px-2.5 py-1.5 text-[11px] font-semibold text-violet-700">
                              📊 KPI Inbox của hội thoại này tính vào Tuần{" "}
                              {getWeekNumber(
                                new Date(inboxKpiWeekDates[openConv]),
                              )}
                            </div>
                          ) : (
                        ...
                        {!verifiedConvIds?.has(openConv) && (
                          <div className="mb-3 text-xs text-muted-foreground">
                            Chọn hội thoại bên trái rồi bấm xác nhận nếu đây là
                        ...
                        (selectedConv.is_customer ||
                          ...
                          <div className="mb-4 flex flex-wrap gap-1.5">
                            {selectedConv.is_customer && (
                              <Badge
                                variant="secondary"
                                className="gap-1 border-transparent bg-emerald-100 text-[10px] text-emerald-700"
                              >
                                <Star size={11} /> Đã đánh dấu khách
                              </Badge>
                            )}
                            {selectedConv.pushed_to_zalo && (
                              <Badge
                                variant="secondary"
                                className="gap-1 border-transparent bg-blue-100 text-[10px] text-blue-700"
                              >
                                <Send size={11} /> Đã đẩy Zalo
                              </Badge>
                            )}
                          </div>
                        ) : !openConv ? (
                        ...
                        {openConv &&
                          ...
                          (verifiedConvIds?.has(openConv) ? (
                            <div className="whitespace-nowrap rounded-xl border border-emerald-200/50 bg-emerald-100/50 p-3 text-center text-xs font-semibold text-emerald-700">
                              Bạn đã đánh dấu Lead cho hội thoại này (Tuần{" "}
                              {confirmedWeekNumber}).
                            </div>
                          ) : (
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  showToastKpi("Lỗi xác nhận lead", false);
                                }
                              }}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="h-2 overflow-hidden rounded-full bg-muted">
                        <div
                          className={cn("h-full", String(color))}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
      ...
      {toast && (
        <div
          className={cn(
            "fixed bottom-6 right-6 z-50 rounded-xl px-5 py-3.5 font-semibold text-white shadow-lg",
      ...
      {kpiToast && (
        <div
          className={cn(
            "fixed bottom-6 right-6 z-50 rounded-xl px-5 py-3.5 font-semibold text-white shadow-lg",
            kpiToast.ok ? "bg-green-600" : "bg-red-600",
          )}
        >
          {kpiToast.msg}
        </div>
