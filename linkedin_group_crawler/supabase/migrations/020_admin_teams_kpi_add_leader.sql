-- ============================================================
-- Migration 020: Them KPI cua chinh LEADER vao RPC
-- get_admin_teams_kpi_overview (trang /admin/teams-management).
--
-- Boi canh: Thanh phan anh "group seeding thieu KPI cua leader" —
-- xac nhan qua doc migration 019: khoi 'kpi_data' CHI join qua
-- public.member_of_teams (danh sach THANH VIEN thuong cua team),
-- con leader (teams.id_leader) chi duoc dung de hien leader_email
-- tren tung dong, KHONG BAO GIO co rieng 1 dong kpi_data cho chinh
-- leader ca. Neu leader cung tu lam lead/inbox/comment/post (nhu
-- 1 nguoi trong team) thi so lieu do bi "mat tich" hoan toan tren
-- trang nay.
--
-- Fix: gop 2 nguon "nguoi" truoc khi join kpi_tracker/actuals —
-- (1) thanh vien thuong qua member_of_teams (nhu cu), (2) leader
-- cua team (teams.id_leader), danh dau bang cot moi is_leader de
-- FE biet duong nao la dong cua leader ma hien tach rieng/gan nhan
-- neu can. Giu nguyen 100% logic tinh KPI/actual (LATERAL+LIMIT 1
-- chong dem 2 lan tu 018, Zalo tu 019) - chi doi NGUON nguoi duoc
-- join, khong doi cach tinh.
-- ============================================================
    ...
    RETURN jsonb_build_object(
        ...
        'kpi_data', COALESCE((
            ...
            FROM (
                ...
                FROM (
                    -- (1) Thanh vien thuong cua team, nhu cu qua member_of_teams
                    SELECT
                    ...
                    -- (2) Moi: chinh leader cua team - TRUOC DAY khong co dong nao
                    -- ca trong kpi_data, KPI/actual cua leader bi "mat tich".
                    SELECT
                        tm.id                              AS team_id_raw,
                        tm.id::text                         AS team_id,
                        tm.name_team                        AS team_name,
                ...
                -- LATERAL + LIMIT 1 (fix migration 018): toi da 1 row kpi_tracker/nguoi
                LEFT JOIN LATERAL (
...
COMMENT ON FUNCTION public.get_admin_teams_kpi_overview IS
    'Admin teams-management RPC. 018: LATERAL+LIMIT 1 chong dem KPI 2 lan. 019: cong them Zalo lead/inbox vao actual. 020: them dong KPI rieng cho chinh leader cua team (truoc day chi co member, leader bi bo sot khoi kpi_data), them cot is_leader de FE phan biet.';
