...
async function syncOldMessages(auth, threadType, threadId, count, timeoutMs, ownId = null) {
  ...
  if (!ownId && typeof api.getOwnId === "function") {
    ownId = api.getOwnId();
  }
...
async function main() {
  ...
  if (api) {
    console.warn("ZCA API zpwServiceMap:", JSON.stringify(api.zpwServiceMap, null, 2));
  }
  ...
  if (args.command === "user-history") {
    const userId = args["user-id"] || args["thread-id"] || input.user_id || input.userId || input.thread_id || input.threadId;
    const count = Math.max(50, Math.min(Number(args.count || input.count || 1000), 3000));

    if (!userId) throw new Error("Missing --user-id");

    const response = await api.getUserChatHistory(String(userId), count);
    const messages = normalizeHistory(response, typeof api.getOwnId === "function" ? api.getOwnId() : null);
  ...
  if (args.command === "sync-sidebar") {
    // Full sidebar sync: groups + friends + pinned + unread + CM recent
    // Returns unified conversation list like Zalo Web sidebar.
    try {
      ...
      // 1. Get all groups IDs
      let groupIds = [];
      try {
        const allGroupsResp = await api.getAllGroups();
        groupIds = groupIdsFromAllGroups(allGroupsResp);
        sidebar.total_groups = groupIds.length;
      } catch (err) {
        sidebar.errors.push(`getAllGroups: ${err && err.message ? err.message : String(err)}`);
      }

      // 2. Get group info in batches of 50
      const groupConversations = [];
      for (let i = 0; i < groupIds.length; i += 50) {
        const chunk = groupIds.slice(i, i + 50);
        try {
          const infoResp = await api.getGroupInfo(chunk);
          const gridInfoMap = infoResp && infoResp.gridInfoMap ? infoResp.gridInfoMap : {};
          for (const [gid, ginfo] of Object.entries(gridInfoMap)) {
            const info = ginfo || {};
            groupConversations.push({
              thread_id: String(gid),
              thread_type: "group",
              name: info.name || info.groupName || info.displayName || gid,
              avatar_url: info.avt || info.avatar || info.fullAvt || null,
              member_count: info.totalMember || 0,
              last_message: null,
              last_message_at: toTimestampMs(info.updateTime || info.lastMsgAt || info.ts) || null,
              unread_count: 0,
              is_pinned: false,
              is_hidden: false,
              is_archived: false,
          ...
          sidebar.errors.push(`getGroupInfo(${chunk.join(",")}): ${err && err.message ? err.message : String(err)}`);
      ...
      try {
        ...
        for (const f of friendList) {
          dmConversations.push({
            thread_id: String(f.userId || ""),
            thread_type: "user",
            name: f.displayName || f.zaloName || f.userName || "",
            avatar_url: f.avatar || f.avatarUrl || null,
            member_count: 0,
            last_message: null,
            last_message_at: null,
            unread_count: 0,
        ...
        sidebar.total_dm = dmConversations.length;
      } catch (err) {
        sidebar.errors.push(`getAllFriends: ${err && err.message ? err.message : String(err)}`);
      }

      // 4. Get pinned conversations
      const pinnedSet = new Set();
      try {
        const pinResp = await api.getPinConversations();
        const pinList = (pinResp && pinResp.conversations) || [];
        for (const pin of pinList) {
          pinnedSet.add(String(pin));
        }
      } catch (err) {
        sidebar.errors.push(`getPinConversations: ${err && err.message ? err.message : String(err)}`);
      }

      // 5. Get unread marks
      const unreadMap = {};
      try {
        const unreadResp = await api.getUnreadMark();
        const unreadData = unreadResp && unreadResp.data ? unreadResp.data : unreadResp;
        const convsGroup = (unreadData && unreadData.convsGroup) || [];
        const convsUser = (unreadData && unreadData.convsUser) || [];
        for (const mark of convsGroup) {
          unreadMap[`g${mark.id}`] = mark;
        }
        for (const mark of convsUser) {
          unreadMap[`u${mark.id}`] = mark;
        }
      } catch (err) {
        sidebar.errors.push(`getUnreadMark: ${err && err.message ? err.message : String(err)}`);
      }

      // 6. Get hidden conversations
      const hiddenSet = new Set();
      try {
        const hiddenResp = await api.getHiddenConversations();
        const hiddenList = (hiddenResp && hiddenResp.conversations) || [];
        for (const hid of hiddenList) {
          hiddenSet.add(String(hid));
        }
      } catch (_err) {}

      // 7. Get archived conversations
      const archivedSet = new Set();
      try {
        const archivedResp = await api.getArchivedChatList();
        const archivedList = (archivedResp && archivedResp.items) || [];
        for (const arc of archivedList) {
          const arcId = arc && (arc.threadId || arc.id || arc.groupId);
          if (arcId) archivedSet.add(String(arcId));
        }
      } catch (_err) {}

      // 8. Try CM API (getrecentv2) for sidebar messages — with retry
      let cmConversations = null;
      if (typeof api.getCMRecent === "function") {
        const CM_SIDEBAR_RETRIES = [1000, 3000]; // 1s, 3s
        for (let attempt = 0; attempt <= CM_SIDEBAR_RETRIES.length; attempt++) {
          try {
            cmConversations = await api.getCMRecent(50, 0);
            break; // success
          } catch (err) {
            const errMsg = err && err.message ? err.message : String(err);
            if (/login|cookie|session|401|403/i.test(errMsg)) {
              sidebar.errors.push(`getCMRecent: ${errMsg}`);
              break; // auth error, don't retry
            }
            if (/429|too many|rate.?limit/i.test(errMsg)) {
              sidebar.errors.push(`getCMRecent: rate limited (429)`);
              // Wait 15s then give up — sidebar doesn't need CM data urgently
              await new Promise(r => setTimeout(r, 15000));
              break;
            }
            if (attempt < CM_SIDEBAR_RETRIES.length) {
              await new Promise(r => setTimeout(r, CM_SIDEBAR_RETRIES[attempt]));
            } else {
              sidebar.errors.push(`getCMRecent: ${errMsg} (after ${attempt + 1} attempts)`);
            }
          }
        }
      }
      ...
      for (const conv of allConversations) {
        const prefix = conv.thread_type === "group" ? "g" : "u";
        const key = `${prefix}${conv.thread_id}`;
        conv.is_pinned = pinnedSet.has(key) || pinnedSet.has(conv.thread_id);
        conv.is_hidden = hiddenSet.has(key) || hiddenSet.has(conv.thread_id);
        conv.is_archived = archivedSet.has(key) || archivedSet.has(conv.thread_id);

        const unreadMark = unreadMap[key];
        if (unreadMark) {
          conv.unread_count = 1;
          conv.unread_ts = unreadMark.ts;
        }
      }

      // 10. If CM data available, merge last messages
      if (cmConversations && Array.isArray(cmConversations.conversations)) {
        const cmMap = {};
        for (const cmConv of cmConversations.conversations) {
          if (cmConv.threadId) {
            cmMap[cmConv.threadId] = cmConv;
          }
        }
        for (const conv of allConversations) {
          const cmConv = cmMap[conv.thread_id];
          if (cmConv) {
            if (cmConv.lastMsgs && cmConv.lastMsgs.length > 0) {
              const lastMsg = cmConv.lastMsgs[cmConv.lastMsgs.length - 1];
              conv.last_message = textOf(lastMsg.content) || conv.last_message;
              conv.last_message_at = toTimestampMs(lastMsg.ts) || conv.last_message_at;
              conv.last_sender_id = lastMsg.uidFrom || null;
              conv.last_sender_name = lastMsg.dName || null;
            }
            if (cmConv.updateTime) {
              conv.last_message_at = conv.last_message_at || toTimestampMs(cmConv.updateTime);
            }
          }
        }
      }

      // 11. Sort like Zalo sidebar: pinned first, then by last_message_at DESC
      allConversations.sort((a, b) => {
        const pinA = a.is_pinned ? 1 : 0;
        const pinB = b.is_pinned ? 1 : 0;
        if (pinA !== pinB) return pinB - pinA;

        const tA = toTimestampMs(a.last_message_at) || 0;
        const tB = toTimestampMs(b.last_message_at) || 0;
        if (tA !== tB) return tB - tA;
      ...
      sidebar.cm_available = cmConversations !== null;
      emitAndExit({ ok: true, ...sidebar }, 0);
      return;
    } catch (err) {
      fail(err);
      return;
  ...
  if (args.command === "sync-cm-messages") {
    // Per-conversation message sync via CM API (getoldv2)
    // Supports pagination with globalMsgId cursor
    // Circuit breaker: skip CM if it failed too many times recently
    try {
      const threadId = args["thread-id"] || input.thread_id || input.threadId;
      const count = Number(args.count || input.count || 50);
      const globalMsgId = args["cursor"] || input.cursor || input.globalMsgId || 0;
      const isGroup = args.type !== "user" && args.type !== "0";
      const ownId = typeof api.getOwnId === "function" ? api.getOwnId() : null;
      ...
      let hasMore = false;
      let cmUsed = false;
      let cmCircuitOpen = false;

      // Circuit breaker state (global for this process)
      if (!global.__cmCircuitBreaker) {
        global.__cmCircuitBreaker = { failures: 0, lastFailAt: 0, cooldownMs: 300000 }; // 5min cooldown
      }
      const cb = global.__cmCircuitBreaker;
      const now = Date.now();
      if (cb.failures >= 3 && (now - cb.lastFailAt) < cb.cooldownMs) {
        cmCircuitOpen = true;
        console.warn(`CM circuit breaker OPEN (${cb.failures} failures, cooldown ${Math.round((cb.cooldownMs - (now - cb.lastFailAt)) / 1000)}s remaining)`);
      ...
      // Try CM API with retry (unless circuit is open)
      if (!cmCircuitOpen && typeof api.getCMOld === "function") {
        const CM_RETRY_DELAYS = [2000, 5000]; // 2s, 5s
        for (let attempt = 0; attempt <= CM_RETRY_DELAYS.length; attempt++) {
          try {
            const cmResp = await api.getCMOld(String(threadId), globalMsgId, count, isGroup);
            const msgList = cmResp && (cmResp.msgs || cmResp.groupMsgs || []);
            if (Array.isArray(msgList) && msgList.length > 0) {
              messages = msgList.map((raw, idx) => normalizeMessage(raw, idx, ownId));
              hasMore = cmResp.hasMore !== false && msgList.length >= count;
              cmUsed = true;
              // CM succeeded — reset circuit breaker
              cb.failures = 0;
              cb.lastFailAt = 0;
            }
            break; // Success or empty result — stop retrying
          } catch (cmErr) {
            const errMsg = cmErr && cmErr.message ? cmErr.message : String(cmErr);
            // Auth errors: don't retry, fail fast
            if (/login|cookie|session|401|403/i.test(errMsg)) {
              throw cmErr;
            }
            // Rate limit: record and use longer cooldown
            if (/429|too many|rate.?limit/i.test(errMsg)) {
              cb.failures += 1;
              cb.lastFailAt = Date.now();
              cb.cooldownMs = Math.min(600000, cb.cooldownMs * 1.5); // Increase cooldown up to 10min
              console.warn(`CM API rate limited (429). Circuit breaker failures=${cb.failures}. Cooldown=${Math.round(cb.cooldownMs / 1000)}s`);
              // Wait 30s before fallback (don't hammer)
              await new Promise(r => setTimeout(r, 30000));
              break;
            }
            // Transient error: retry with backoff
            console.warn(`CM API attempt ${attempt + 1} failed: ${errMsg}`);
            if (attempt < CM_RETRY_DELAYS.length) {
              await new Promise(r => setTimeout(r, CM_RETRY_DELAYS[attempt]));
            } else {
      ...
      // Fallback: HTTP history API
      if (!cmUsed) {
        try {
          const histFn = isGroup ? api.getGroupChatHistory : api.getUserChatHistory;
          if (typeof histFn === "function") {
            const histResp = await histFn(String(threadId), count);
            messages = normalizeHistory(histResp, ownId);
            hasMore = histResp && histResp.more === 1;
          }
        } catch (_histErr) {
          // HTTP history also failed
        }
      }

      // Sort oldest → newest
      messages = sortMessagesOldToNew(messages);
      ...
      emitAndExit({
        ...
        message_count: messages.length,
        has_more: hasMore,
        cursor_next: oldestMsg ? (oldestMsg.raw && oldestMsg.raw.globalMsgId) || oldestMsg.message_id : null,
        thread_id: threadId,
        source: cmUsed ? "cm-getoldv2" : (cmCircuitOpen ? "http-history(cm-circuit-open)" : "http-history"),
        cm_circuit: { failures: cb.failures, cooldown_s: Math.round(cb.cooldownMs / 1000) },
      }, 0);
      return;
    } catch (err) {
      fail(err);
  ...
  if (args.command === "sync-old-messages") {
    ...
    const result = await syncOldMessages(auth, type, threadId ? String(threadId) : "", count, timeoutMs, null);
    ...
    try {
      ...
      if (includeFriends && summary.friends.length > 0) {
        ...
        for (let i = 0; i < friendIds.length; i += CONCURRENCY) {
          ...
          const results = await Promise.allSettled(
            chunk.map(async (fid) => {
              // ZCA getUserChatHistory dùng để lấy lịch sử DM cá nhân
              const resp = await api.getUserChatHistory(fid, messagesPerChat);
              return { fid, resp };
            }),
          );
          for (const r of results) {
