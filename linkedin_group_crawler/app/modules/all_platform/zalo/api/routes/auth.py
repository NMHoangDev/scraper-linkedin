...
async def sync_and_start_listener(user_id: str, auth: dict) -> None:
    import time as _time
    t0 = _time.time()
    try:
        logger.info(f"Background sync: [1/2] starting first-time sync for user={user_id}...")
        ...
        result = await first_time_sync(
            auth=auth,
            zalo_account_id=user_id,
            messages_per_chat=50,
            group_limit=8,
            include_friends=True,
        )
        
        # 1. Upsert conversations (groups + friends) to database
        groups = result.get("groups") or []
        friends = result.get("friends") or []
        all_chats = groups + friends
        if all_chats:
            await upsert_groups(user_id, all_chats)
            logger.info(f"Saved {len(all_chats)} conversations to database for user={user_id}")
            
        # 2. Save message history to database
        raw_msgs = result.get("messages") or []
        if raw_msgs:
            messages = []
            for row in raw_msgs:
                try:
                    # Map raw dict to Message object
                    msg = Message(
                    ...
                    logger.warning(f"Error mapping synced message row: {map_exc}")
                    
            if messages:
                saved_count = await save_global_listener_messages(user_id, messages)
                logger.info(f"Saved {saved_count}/{len(messages)} historical messages to database for user={user_id}")
                
        dt = _time.time() - t0
        logger.info(
            f"Background sync: [1/2] first-time sync done for {user_id} in {dt:.1f}s"
        )
    except Exception as sync_exc:
        dt = _time.time() - t0
        logger.warning(
            f"Background sync: [1/2] first-time sync FAILED for {user_id} after {dt:.1f}s: {sync_exc}. "
            f"Continuing to start listener so user can receive live messages."
        )
    finally:
        t1 = _time.time()
        try:
            logger.info(f"Background sync: [2/2] starting persistent listener for user={user_id}...")
            await start_listener(user_id, auth, force_restart=True)
            dt = _time.time() - t1
            logger.info(
                f"Background sync: [2/2] persistent listener started for user={user_id} in {dt:.1f}s. "
                f"Total time: {_time.time() - t0:.1f}s."
            )
        except Exception as start_exc:
            dt = _time.time() - t1
            logger.warning(
                f"Background sync: [2/2] could not start listener for user={user_id} after {dt:.1f}s: {start_exc}"
            )
...
async def _build_current_status_payload(request: Request, user_id: str) -> dict:
    ...
    if await ensure_session_zca_auth(session):
        ...
        try:
            listener = get_listener_status(user_id)
            if not listener.get("running") and not listener.get("auth_expired") and session.zca_auth:
                logger.info(f"Auto-starting offline listener for confirmed user={user_id} on status check")
                asyncio.create_task(start_listener(user_id, session.zca_auth))
        except Exception as exc:
            logger.warning(f"Could not auto-start offline listener in current-status payload for user={user_id}: {exc}")
...
async def _handle_zca_qr_events(session_id: str) -> None:
    while True:
        ...
        if event_type == "success":
            ...
            # Run background first-time sync (populates DB conversations and messages) then start persistent listener
            asyncio.create_task(sync_and_start_listener(session.user_id, auth))
...
async def import_session_from_extension(
    ...
    asyncio.create_task(sync_and_start_listener(user_id, auth))
