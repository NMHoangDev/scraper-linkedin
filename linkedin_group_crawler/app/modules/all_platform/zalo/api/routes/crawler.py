...
from app.modules.all_platform.zalo.crawler.scroll_handler import scroll_and_collect, verify_group_for_crawl
from app.modules.all_platform.zalo.schemas.job import JobData, JobProgress
from app.modules.all_platform.zalo.schemas.session import SessionData
from app.modules.all_platform.zalo.services.gsheet_service import write_messages
from app.modules.all_platform.zalo.services.job_store import get_job, save_job
    ...
    *,
    ...
    for candidate_id in candidates:
        try:
            ...
            is_friend = not str(candidate_id).startswith("g")
    ...
    try:
        ...
        if zca_auth:
            ...
            if not messages:
                ...
                try:
                    is_friend = not str(resolved_group_id).startswith("g")
                    if is_friend:
                        messages = await get_zca_user_history(
                            zca_auth,
                            resolved_group_id,
