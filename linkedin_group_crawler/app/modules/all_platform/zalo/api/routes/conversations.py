...
async def get_conversations(
    ...
    # Auto-start persistent listener if it is offline
    try:
        from app.modules.all_platform.zalo.services.zca_persistent_listener import (
            get_listener_status,
            start_listener,
        )
        st = get_listener_status(user_id)
        if not st.get("running") and not st.get("auth_expired"):
            auth = await load_zca_auth(user_id)
            if auth:
                logger.info(f"Auto-starting offline listener for account_id={user_id} on conversations fetch")
                asyncio.create_task(start_listener(user_id, auth))
    except Exception as exc:
        logger.warning(f"Could not auto-start offline listener in get_conversations for user={user_id}: {exc}")
...
async def sync_recent_conversations(
    ...
    # Run global user/group sync in parallel via WebSocket
    try:
        user_sync_count = max(300, len(friends_to_sync) * 20)
        group_sync_count = max(300, len(groups_to_sync) * 20)
        
        user_msgs, group_msgs = await asyncio.gather(
            sync_zca_group_old_messages(auth, thread_type=1, count=user_sync_count),
            sync_zca_group_old_messages(auth, thread_type=2, count=group_sync_count),
        )
    except ZcaAuthExpiredError:
        raise HTTPException(status_code=401, detail=ZCA_SESSION_EXPIRED_DETAIL)
    except Exception as exc:
        logger.error(f"Global WebSocket sync failed: {exc}")
        raise HTTPException(status_code=500, detail=f"Không thể đồng bộ tin nhắn qua WebSocket: {exc}")

    all_messages = user_msgs + group_msgs

    # Group messages by group_id/thread_id
    msgs_by_group: Dict[str, List[Message]] = {}
    for msg in all_messages:
        gid = msg.group_id
        if gid:
            msgs_by_group.setdefault(gid, []).append(msg)
    ...
    all_chats_to_process = groups_to_sync + friends_to_sync
    for chat in all_chats_to_process:
        gid = chat.group_id
        chat_msgs = msgs_by_group.get(gid)
        ...
        try:
            ...
            logger.warning(f"Failed saving synced messages for group={gid}: {exc}")
            results.append(
                SyncRecentGroupResult(
                    group_id=gid,
                    group_name=chat.name,
    ...
    # Combine skipped and active sync results
    results = skipped_results + friends_skipped + results
...
async def _background_sync_conversation_messages(account_id: str, conversation_id: str):
    ...
    try:
        ...
        is_group = conversation_id.strip().startswith("g")
        if is_group:
        ...
        if messages:
            ...
            try:
                ...
                register_account_owner(account_id, account_id)
                shared_ids = await list_shared_conversation_ids(account_id)
                event = {
                    "type": "new_messages",
                ...
                logger.info(f"Published background sync messages for {conversation_id} to SSE stream")
            except Exception as sse_exc:
                logger.warning(f"Could not publish background sync messages to SSE: {sse_exc}")
    except Exception as exc:
...
# ─────────────────────────────────────────────────────────────────────
# Sidebar Sync + CM Message Sync Routes
# ─────────────────────────────────────────────────────────────────────

class SyncSidebarRequest(BaseModel):
    account_id: Optional[str] = None


@router.post("/sync-sidebar")
async def sync_sidebar_route(
    body: SyncSidebarRequest,
    x_user_id: str = Header("default", alias="X-User-ID"),
):
    """Sync full sidebar: groups + friends + pinned + unread + CM recent.

    Returns unified conversation list matching Zalo Web sidebar.
    """
    user_id = _normalize_user_id(body.account_id or x_user_id)
    auth = await load_zca_auth(user_id)
    if not auth:
        raise HTTPException(status_code=404, detail="Chưa đăng nhập Zalo cho tài khoản này.")

    try:
        result = await sync_sidebar(auth, zalo_account_id=user_id)
        ...
        for conv in conversations:
            ...
            try:
                ...
                errors.append(f"save_{conv['thread_id']}: {exc}")
        ...
        return {
            ...
            "total_dm": result.get("total_dm", 0),
            "cm_available": result.get("cm_available", False),
            "saved_count": saved_count,
            "sync_errors": result.get("errors", []) + errors,
        }
    except ZcaAuthExpiredError:
        raise HTTPException(status_code=401, detail=ZCA_SESSION_EXPIRED_DETAIL)
    except asyncio.TimeoutError:
        raise HTTPException(status_code=504, detail="Sidebar sync timed out.")
    except Exception as exc:
        logger.exception(f"Sidebar sync failed: {exc}")
        raise HTTPException(status_code=500, detail=f"Sidebar sync error: {exc}")


class SyncCMMessagesRequest(BaseModel):
    count: int = Field(default=50, ge=1, le=200)
    cursor: Optional[str] = None
    is_group: bool = True


@router.post("/sync-messages/{thread_id}")
async def sync_cm_messages_route(
    thread_id: str,
    body: SyncCMMessagesRequest,
    account_id: Optional[str] = Query(None),
    x_user_id: str = Header("default", alias="X-User-ID"),
):
    """Sync messages for a specific conversation via CM API.
    ...
    try:
        ...
        if raw_messages:
            try:
                messages_to_save = [_to_message_from_raw(m, thread_id) for m in raw_messages]
                await save_listener_messages(user_id, thread_id, messages_to_save)
                saved_count = len(messages_to_save)
            except Exception as exc:
                logger.warning(f"Failed to save CM messages: {exc}")
