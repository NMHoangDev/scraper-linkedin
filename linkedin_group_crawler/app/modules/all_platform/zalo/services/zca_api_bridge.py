...
async def get_zca_group_history(
    ...
    *,
    ...
    try:
        ...
        logger.warning(f"CM history sync failed for group={group_id}, fallback to WS old sync: {exc}")
        return await sync_zca_group_old_messages(
            auth,
            group_id=group_id,
            thread_type=2,
            count=count,
...
async def get_zca_user_history(
    ...
    *,
    ...
    try:
        ...
        logger.warning(f"CM history sync failed for user={user_id}, fallback to WS old sync: {exc}")
        return await sync_zca_group_old_messages(
            auth,
            group_id=user_id,
            thread_type=1,
...
async def sync_sidebar(
    ...
    *,
    ...
    Returns a unified conversation list matching Zalo Web sidebar.

    Returns:
        Dict with keys: ok, conversations, errors, own_id, total_groups,
        total_dm, cm_available, source.
    ...
    conv_count = len(result.get("conversations") or [])
    logger.info(
        f"ZCA sidebar sync done for user={zalo_account_id}: "
        f"conversations={conv_count}, "
        f"groups={result.get('total_groups', 0)}, "
        f"dm={result.get('total_dm', 0)}, "
        f"cm={result.get('cm_available', False)}, "
        f"errors={len(result.get('errors') or [])}"
    )
    return result


async def sync_cm_messages(
    auth: Dict[str, Any],
    thread_id: str,
    *,
    count: int = 50,
    ...
    """Sync messages for a specific conversation via CM API (getoldv2).

    Supports pagination via cursor (globalMsgId).

    Args:
        auth: ZCA auth dict.
        thread_id: conversation/thread ID.
        count: number of messages to fetch.
        cursor: pagination cursor (globalMsgId from previous response).
        is_group: whether thread is a group conversation.

    Returns:
        Dict with keys: ok, messages, message_count, has_more,
        cursor_next, thread_id, source.
    ...
    logger.info(
        f"ZCA CM message sync: thread={thread_id}, "
        f"messages={msg_count}, source={source}, "
