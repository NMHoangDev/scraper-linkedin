# Module CRM độc lập — deploy cho CloudGate (`instance=cloudgate`)

> Đây là bản clone của `crm-module/` (deploy cho Markee) — CÙNG code, chỉ khác
> `backend/.env`'s `CRM_INSTANCE=cloudgate` (đọc/ghi riêng dữ liệu CloudGate
> trên cùng 1 DB self-host). Xem mục "Multi-tenant" bên dưới để hiểu cơ chế,
> và mục "Deploy thêm 1 instance mới" nếu cần tạo thêm brand khác sau này.
