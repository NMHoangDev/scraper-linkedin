...
async def create_broadcast(
    ...
    # 1. Try to load ZCA auth from disk first
    from app.modules.all_platform.zalo.services.zca_auth_store import load_zca_auth
    zca_auth = await load_zca_auth(user_id)
    session_id = None

    if not zca_auth:
        # 2. Fall back to active Playwright session in memory
    ...
    try:
        ...
        for msg in messages:
            ...
            if msg.get("source_message_id"):
                found_ids.add(msg["source_message_id"])

        if not all(mid in found_ids for mid in body.message_ids):
        ...
        db_message_ids = [msg["id"] for msg in messages]
...
async def _run_broadcast(
    ...
    try:
        async with _BROADCAST_WORKER_LOCK:
            ...
            # Check ZCA auth from disk first
            from app.modules.all_platform.zalo.services.zca_auth_store import load_zca_auth
            zca_auth = await load_zca_auth(user_id)
            if zca_auth:
            ...
            # Fallback to Playwright session
            if not session_id:
                raise RuntimeError("No active Playwright session and no ZCA auth found")
