"""Supabase-based Quick Comment template library service for all-platform module."""
...
TABLE = "quick_comment_templates"


def get_all_quick_comments(platform: str | None = None) -> list[dict]:
    """Get all quick comment templates, optionally filtered by platform (also includes 'all')."""
    supabase: Client = get_supabase_client()

    query = supabase.table(TABLE).select("*")
    if platform:
        query = query.in_("platform", [platform, "all"])
    result = query.order("order_index").execute()
    return result.data or []


def add_quick_comment(payload: dict, created_by: str | None) -> dict:
    """Add a new quick comment template, appended to the end of the order."""
    supabase: Client = get_supabase_client()
...
def update_quick_comment(template_id: str, payload: dict) -> dict:
...
def delete_quick_comment(template_id: str) -> dict:
...
def reorder_quick_comment(template_id: str, direction: str) -> list[dict]:
